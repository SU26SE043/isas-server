...
services:
  ...
  isas.interviewservice:
    ...
    environment:
      ...
      # B2C coaching (2026-09-17, BC-6 ngoại lệ) — retention ảnh webcam kiểm mặt. TẮT tường minh,
      # mirror BK25 bên campaignservice ngay dưới: JOB XOÁ DỮ LIỆU SINH TRẮC HỌC.
...
- [ ] **AC1/B5 — `multi_voice` GIỮ TẮT**: `MULTI_VOICE_ENABLED` (aiworker) mặc định **`false`**, và **chưa đủ bằng chứng để bật** (tính tới 2026-08-28). Hiệu chuẩn đạt 0 dương-tính-giả trên 25 ca âm + bắt 2/2 ca dương, nhưng **toàn bộ ghi âm thật hiện có chỉ của MỘT người** (13 bản, F0 116–158 Hz) và **mọi ca dương đều là giọng tổng hợp `say`** ⇒ chưa ai từng đo tính năng này trên hai người thật. Đầu ra là một **cáo buộc gian lận** gắn lên hồ sơ ứng viên mà `session_flags` không có đường phản hồi. Điều kiện bật: thu ≥10 buổi có **hai người thật** + ≥20 buổi một người, hiệu chuẩn lại, rồi mới lật cờ.
      • Env đi kèm: `CAMPAIGN_CALLBACK_BASE=http://isas.campaignservice:8080` (tên container, **không** dấu `/` cuối). Rỗng ⇒ detector bỏ qua kèm cảnh báo trong log.
      • ⚠ Chỉ đặt ở **`aiworker`**, không phải `aiapi` — detector chạy trên đường chấm bất đồng bộ, KHÔNG chạm `/decide-next` (đường đó đồng bộ trong request upload, đã đo 96s trên bài dài / trần 90s).
      • ⚠ Cờ này chỉ mở cổng phía AIService. Cờ tới được HR còn cần **`campaign.anti_cheat_enabled`** bật: `multi_voice` KHÔNG nằm trong `IdentitySignals` nên `face_verify_enabled` một mình **không đủ** để `SessionFlagController` lưu nó.
      • Model nhúng giọng (28 MB) tải **lúc build image** (`Dockerfile`, ghim URL + sha256) ⇒ image ~594 MB → ~622 MB. Không tải lúc chạy, không kéo torch.
