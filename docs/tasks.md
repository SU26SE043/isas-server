...
| D1 | `campaign_invitations` + magic-link token (mời thẳng qua list email) | `POST …/invitations {emails[]}` → **validate định dạng/dedup/cap `max_candidates`** → rows invitation + email queue; email sai định dạng → trả `failed[]` per-item, **KHÔNG chặn cả batch**; vượt cap → 4xx | C7 | not_started |
...
| BC13 | AIService 3 endpoint roadmap (sync, stateless) | `POST /generate-roadmap {jobCategory, level, weaknesses?, cvText?}` → `{milestones[{title,focusCriteria[],lessons[{title}]}]}`; `POST /generate-lesson-theory` → `{theoryMarkdown}` (tiếng Việt); `POST /summarize-roadmap` → `{strengths[],weaknesses[],improvements[],overallComment}`; AI **không ghi DB** | — | not_started · spec: [ai.md](services/ai.md) §Roadmap ôn tập B2C |
