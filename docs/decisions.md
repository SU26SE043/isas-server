...
## D24 — ⚠ SỐ BỊ BỎ TRỐNG, quyết định CHƯA ĐƯỢC VIẾT (ghi nhận 2026-08-07)
**Trạng thái:** chưa có nội dung trong file này. Đây **không phải** lỗi đánh máy — nhiều nơi đã trích `D24` như một quyết định có thật, nên để trống mà không giải thích sẽ khiến người sau tưởng mình đọc sót.
**Ai đang trích:** [progress.md](progress.md) §2026-07-18 (*"PAY-8+D24 tiền THẬT"*) và bản nháp trên nhánh **chưa merge** `docs/d24-payos-webhook-via-gateway` (`bc465aa`, docs-only, `merge-base --is-ancestor` → **1**).
**Nội dung dự kiến** (theo nhánh nháp, **CHƯA ai duyệt**): webhook PayOS **ĐI QUA gateway** — đảo vế "webhook PayOS KHÔNG qua gateway" của **GEN-1** — vì prod chỉ có một tunnel công khai trỏ gateway, cổng `5271` không có đường ra Internet, và lá chắn thật của webhook là **HMAC-SHA256 verify trước mọi thay đổi trạng thái** (PAY-8) chứ không phải network topology.
**Việc cần làm:** team chốt → hoặc merge nhánh ratify (kèm sửa `rules.md` GEN-1 + XML doc `WebhookController.cs:12`), hoặc bác và dựng ingress riêng cho `5271`. Tracking: `R6` trong [tasks.md](tasks.md) §S10. 🛑 **Đừng chặn `webhook/**` ở gateway trước khi trỏ lại URL bên PayOS** — webhook sẽ chết im lặng vì polling P3 vẫn cộng credit.
⚠ Kèm: **D25/D26 và D27/D28 đang lệch thứ tự** trong file này (D26 đứng trước D25, D28 trước D27) — thuần trình bày, nội dung không sao.
...
> **BK26 (2026-07-26):** token vẫn không cần mật khẩu, nhưng thao tác redeem/join nay bắt buộc JWT `Candidate` có email khớp lời mời; đây là lớp định danh bổ sung, không đảo quyết định D25 về mật khẩu.
...
## D28 — Tiered subscription: B2C metered, B2B entitlement-only (2026-08-01)
**Quyết định:** catalog Payment có `plans.audience` là rào B2C/B2B. B2C Plus/Pro dùng quota buổi theo tháng (`SubscriptionMetered`) rồi mới fallback credit pack; Free dùng credit/trial. B2B Starter/Business/Enterprise chỉ mở feature và các cap campaign/candidate/seat, mỗi buổi vẫn tiêu org-credit hoặc postpaid đã được Admin duyệt. Không seed hoặc phát hành Unlimited; guard `Tiering:AllowUnlimitedPlans=false` mặc định.

**Vì sao:** quota thuộc thuê bao, không thuộc ví credit. Meter sẽ gắn subscription và reservation snapshot đúng meter/period để release hoặc consume không bị đổi nghĩa khi upgrade hay qua tháng. JWT không mang tier; Payment resolve entitlement nội bộ rồi Interview/Campaign stamp hoặc gate theo feature, Payment tự quyết money gate.

**Hệ quả:** triển khai additive dưới feature flag `Tiering:Enabled=false`; snapshot entitlement lúc kích hoạt, không hồi tố khách hiện hữu. Chi tiết giao thức/schema/task: [payment.md](services/payment.md) và [tasks.md](tasks.md) T1–T13.

## D27 — GROUNDING: AI phỏng vấn dựa nguồn uy tín (RAG lớp SINH, admin curate) (2026-08-01)
**Quyết định:** ground **lớp SINH** của AI (câu hỏi · lý thuyết bài học · roadmap) vào **corpus tài liệu uy tín do admin curate**; mỗi nội dung sinh trả **citation `chunkId`** để bấm ra nguồn thật. Kiến trúc: kho `knowledge_sources` (Postgres, InterviewService) + vector chunk trong **Qdrant** (service riêng); **AIService thêm `/embed` stateless** (giữ GEN-4 write-free) + inject block "TÀI LIỆU THAM CHIẾU" (hardcode, không cho F21 override) và trả `citedChunkIds` ⊆ tập cấp; **retrieval ở InterviewService** truyền grounding vào request sinh (mẫu rubric truyền cho scoring). Embedder **`gemini-embedding-001`** (đa ngôn ngữ, `output_dimensionality=768`, `task_type` RETRIEVAL_DOCUMENT/QUERY). Ingest nguồn qua **Context7** (BE/FE) + dán tay (BA). **3 trạng thái output**: `grounded` (có cite) · `ungrounded` (retrieval miss/Qdrant-down → VẪN sinh + nhãn "chưa có nguồn") · `blocked` (injection/ngoài phạm vi, hiếm, không build detector Phase 1). Roadmap dùng **precompute** (batch-embed lúc tạo roadmap → `roadmap_lessons.grounding_refs`, không retrieve realtime lúc mở bài). Chunking `IChunker` theo `source_type`.
**Vì sao:** AI hiện 100% trí nhớ Gemini, không nguồn → không defensible + không có điểm khác biệt. "Wow" = minh bạch nguồn + **đo được** ít bịa hơn. Citation `chunkId`-referenced khiến model **không bịa URL được** (khác trò cũ "đoán URL rồi lọc allowlist"). Embedder đa ngôn ngữ xử **recall xuyên ngôn ngữ** (sản phẩm VN, corpus EN) ở gốc, không cần dịch query.
**Bị loại:** (a) **ground lớp CHẤM realtime** — độ trễ × self-consistency + rủi ro model bị dẫn dắt; giữ "AI gợi ý + HR quyết", faithfulness đo offline (Phase 2). (b) **pgvector** — corpus nhỏ, Qdrant tách kho gọn hơn + đúng DB-per-service (đánh đổi: thêm 1 service + đồng bộ 2 kho, xử bằng delete-Qdrant-trước + sweep). (c) **bật self-consistency N=3** — 3× cost/độ trễ/storage đổi lợi ích mỏng; chỉ giảm phương sai không giảm bias → giữ **N=1**. (d) **BABOK/PMBOK/sách bản quyền** vào corpus — bản quyền; BA dùng nguồn công khai (Scrum Guide/OMG/IREB/Atlassian/NN·g), khai độ phủ thật. (e) **query-translation VN→EN** làm giải pháp chính — chỉ giữ làm backup, embedder đa ngôn ngữ là chính.
**Hệ quả:** feature theo yêu cầu user, **default TẮT** (`GROUNDING_ENABLED=false`) → deploy an toàn tới khi nạp corpus + bật. Phase 2 (đo faithfulness/recall bằng gold set NGƯỜI gán) + sample-answer grounding + orphan-vector sweep = **chưa làm**. Rủi ro cố hữu RAG (retrieval quality chỉ verify được ở L3/đo) khai minh bạch — không hứa quá. Nhánh `s/grounding-integration` (BE) + `feat/grounding-fe` (FE), migration `AddKnowledgeAndGroundingRefs` đã apply prod.
