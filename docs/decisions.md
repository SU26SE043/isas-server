...
**Hệ quả (gap tầng org = follow-up, KHÔNG phải lỗi thiết kế):** **A6b-phase2** invite qua email + **attach account có sẵn** (nay HR có account trước → thêm vào org bị **409**) · **BK14** JWT **không refresh khi đổi role** (PATCH OrgAdmin↔HrMember xong token cũ giữ role cũ tới re-login) · ratify: HrMember vẫn **xem** billing (GET), chỉ mutation bị chặn. Spec: [services/auth.md](services/auth.md) §Organization; rules **AUTH-4/5/6/8**.
