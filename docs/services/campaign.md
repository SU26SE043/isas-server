...
| PUT | `/campaign/{id}/job-needs` | ✅ HR chốt **nhu cầu công việc** dùng để sàng CV (replace-all). Body `[{needId?,category,text,isMustHave?}]`, `category ∈ Technical·WorkStyle·Communication·Growth`. `needId` echo lại để kết quả sàng đã lưu còn trỏ đúng dòng (mẫu F10); **`source` KHÔNG nhận từ client** — server sở hữu. **RNK1 · HĐ-6 — `isMustHave` GIỮ giá trị client** (khác `source`: quyết định nghiệp vụ, không phải nhãn nguồn gốc); AI không đề xuất (`false`). Ngoài `Draft` (hoặc `Active` mà `job_needs` còn rỗng + chưa ai sàng) → **409** — đổi `isMustHave` sau khi có người sàng cũng 409; `category` lạ → **400** |
