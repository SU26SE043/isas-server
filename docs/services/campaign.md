...
- `CampaignResponse.questionBank` (read-time, mọi response): `{ total, alwaysAsked (số câu `isRequired`), questionsPerSession, groups: [{name, count}] (nhóm null/`""` → "Chung", gộp không phân biệt hoa/thường; casing hiển thị = câu sớm nhất của nhóm), warnings: string[] }`. Cảnh báo: K > total · alwaysAsked > K · K×(1+d) > T (ngân sách adaptive, dùng chung `AdaptiveBudgetRule` — HĐ-7).
...
- **Rút đề CHO MỖI ỨNG VIÊN** (`ParticipationService` → `QuestionPoolSelector`, deterministic theo `(campaignId, candidateId)`): giữ hết câu `isRequired` → chia khe còn lại ĐỀU theo nhóm → xáo. Interview nhận đúng K câu (seed = K). `questionGroup` KHÔNG gửi sang Interview (không chấm theo nhóm).
...
| questions_per_session | int? | ✅ **RNK1 · HĐ-8** — K = số câu mỗi ứng viên thi. `null` = thi HẾT bộ (mặc định, chiến dịch cũ). `1..20` (CHECK `ck_campaigns_questions_per_session_positive` chỉ ép `>= 1`; trần 20 ở code — F2b). PUT `0` = RESET về null. Đổi K ngoài `Draft` → 409. `QuestionPoolSelector` rút ĐỀU theo `question_group`, giữ hết câu `is_required` |
