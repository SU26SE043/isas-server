...
| PUT | `/campaign/{id}/questions` | Thay toàn bộ câu hỏi. Body `List<QuestionItem>`. Ngoài `Draft` → **409** (CAMP-18) |
...
`QuestionItem`: `{ id?: uuid, questionText, source: "AiGenerated"|"CustomHr" (server sở hữu — F10, giá trị client bỏ qua), isRequired: bool = true, sampleAnswer?: string (ba trạng thái: vắng = không đổi · `""` = xoá · text = ghi đè), questionGroup?: string (nhóm chủ đề — chỉ có nghĩa khi bật ngân hàng đề; null/`""` = nhóm "Chung") }`. `id` echo lại để GIỮ `source`/`created_at` khi HR sửa (F10).
`CriterionItem` 🔜: `{ id?: uuid, name: string, description?: string, weight: decimal(5,4), maxScore: int, minPct?: int (RNK1 · HĐ-5 — điểm sàn %), levels?: CriterionLevelItem[] }` — tiêu chí **CÓ CẤU TRÚC** HR nhập thẳng (Σweight=1).
...
### `campaign_questions`
`id` · `campaign_id` (FK Cascade) · `org_id` (denormalize theo campaign = ORG, BK4) · `question_text` · `source` (enum `AiGenerated`/`CustomHr` — provenance do SERVER sở hữu, F10; hiển thị nguồn khi HR duyệt) · `is_required` (mặc định true) · `hr_edited_at?` (R10 — mốc HR sửa nội dung câu AI; `source` KHÔNG đổi) · `sample_answer?` (đáp án mẫu HR soạn — ví dụ để AI hiệu chỉnh thang điểm, KHÔNG phải đáp án đúng duy nhất) · `question_group?` (varchar(100) — nhóm chủ đề, RNK1 · HĐ-8) · `created_at`.
