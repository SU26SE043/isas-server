...
- `CampaignResponse.questionBank` (read-time, mọi response): `{ total, alwaysAsked (số câu `isRequired`), questionsPerSession, groups: [{name, count}] (nhóm null/`""` → "Chung", gộp không phân biệt hoa/thường; casing hiển thị = câu sớm nhất của nhóm), warnings: string[], coverageWarnings: [{criterionId, name}] }`. Cảnh báo: K > total · alwaysAsked > K · K×(1+d) > T (ngân sách adaptive, dùng chung `AdaptiveBudgetRule` — HĐ-7) · **SC2 · W1 `K_BELOW_CRITERIA_GROUPS: …`** khi `questionsPerSession = K < số tiêu chí CHÍNH distinct` (tiêu chí chính = `targetCriterionIds[0]`; K null hoặc 0 câu gắn nhãn ⇒ không bắn) — vào `warnings` ⇒ **chặn publish** (D-5). **`coverageWarnings`** (SC2): tiêu chí `WhenTargeted` **không câu nào nhắm** (mọi vị trí nhãn; `Always` không bao giờ vào) — **chỉ cảnh báo, KHÔNG chặn publish**, luôn có mặt (`[]` khi sạch), kể cả trên danh sách campaign (nạp 1 projection WhenTargeted cho cả trang).
...
- **Rút đề CHO MỖI ỨNG VIÊN** (`ParticipationService` → `QuestionPoolSelector`, deterministic theo `(campaignId, candidateId)`): giữ hết câu `isRequired` → chia khe còn lại ĐỀU theo **rổ = tiêu chí CHÍNH** (`targetCriterionIds[0]`, SC2 · W2 — đúng đại lượng INT-18 dùng để chấm), câu chưa gắn nhãn (`null`/`[]`) rơi về `questionGroup` như trước (rổ Guid và rổ tên là hai không gian khoá, không trộn) → xáo. Interview nhận đúng K câu (seed = K). `questionGroup` KHÔNG gửi sang Interview (không chấm theo nhóm).
