...
- Khi `Tiering:Enabled=false`, Campaign trả projection `legacy` cục bộ và **không gọi Payment/không áp cap tier**: đây là rollout additive, giữ nguyên hành vi B2B trước D28. Khi bật, Campaign gọi trực tiếp Payment `GET /internal/entitlements?ownerType=Org&ownerId={orgId}` với `X-Internal-Token`, cache theo org 90 giây. Timeout/non-2xx/JSON hỏng luôn rơi an toàn về **Starter**: 1 Active campaign, 25 candidates, grounding/postpaid tắt — **nhưng adaptive BẬT** (INT-19: mỗi buổi tiêu 1 credit ví org bất kể gói, nên một lần Payment sập không được biến thành lời từ chối SAI với HR đang dùng gói có adaptive).
- Create chặn khi số `Active` đã đạt tier; create/update chặn `maxCandidates` vượt cap hoặc bật grounding không được cấp. **Adaptive KHÔNG bị gói chặn** (INT-19) — tham số vẫn đi qua `ValidateEntitled*` để chữ ký nói đúng thứ HR gửi lên, nhưng không còn là điều kiện từ chối. Campaign đang tồn tại không bị hạ khi tier hết hạn.
...
| POST | `/campaign` | Tạo (Draft). Body `{ title, domain, seniority?, jdText?, criteriaText?, criteria?, maxCandidates?, timeLimitMinutes, antiCheatEnabled, startsAt, expiresAt, questions[] }`. `seniority` là `Fresher`\|`Junior`\|`Middle`\|`Senior` (mặc định `Junior`), HR chọn một mức cho toàn campaign; được snapshot sang session khi candidate Start. **🔜 `jdText`/`criteriaText`** = JD/Criteria dạng text (set `*_text`, `*_file_url=null`); **🔜 `criteria`** = `CriterionItem[]` tiêu chí structured HR khai thẳng. **🔜 `domain` → thay bằng `jobCategory` enum `BA·BE·FE` bắt buộc** (code hiện nhận `domain` free-text đang gánh vai này không kiểm kiểu — xem §DB `campaigns.job_category`). **🔸 `timeLimitMinutes` TẠM BỎ** — vẫn nhận optional cho tương thích nhưng **không enforce** (chỉ giới hạn từng câu) **Lịch mở (2026-09-21):** `startsAt` ở **quá khứ được nhận** (= "mở ngay", cùng nghĩa với PUT và `start-now`; chốt 400 cũ đã gỡ vì FE tạo nháp lười ở bước 2 với giờ mặc định = lúc mở wizard + 1' ⇒ HR điền quá 1' là 400). Vẫn **400** khi `expiresAt` ở quá khứ hoặc `startsAt ≥ expiresAt`. |
...
| PUT | `/campaign/{id}` | Sửa campaign (check ownership). Body có thể gồm **🔜 `jdText?`/`criteriaText?`** (text) và **🔜 `criteria?`** (`CriterionItem[]` structured) để cập nhật/ghi đè JD/Criteria. **CMP4-B2 — `domain` + `language` khoá bởi CÙNG thước với 3 luật lọc cứng (`requiredSkills`/`keywordsAny`/`minYearsExperience`) và với `PUT /job-needs` (CMP4-B1): campaign đã có `cv_submission` NÀO HOẶC `Closed`/`Archived` → **409** (đổi trường quyết định cách AI sàng/chấm sau khi đã đo ai đó thì không so sánh được — không có nhãn phiên bản). Trước CMP4-B2: `domain` KHÔNG có cửa nào; `language` chỉ 409 khi `≠ Draft`. `language` sai thang → **400** (`ValidateLanguage` vẫn chạy). `seniority` vẫn `≠ Draft → 409` riêng (không phải trường sàng CV). **CMP4-B3 — `startsAt` chỉ đổi được khi `Draft`; campaign Active → **409**, đi qua `POST /campaign/{id}/start-now` (đường đó kiểm ca thi + hạn campaign, ghi audit, gửi lại thư). `expiresAt` KHÔNG bị khoá (HR gia hạn campaign là hợp lệ). **SC2 · W1 — `criteria[].scoringScope`** (`"Always"`\|`"WhenTargeted"`, vắng ⇒ `Always`, lạ ⇒ **400** nêu tên tiêu chí) nằm TRONG vân tay thước đo ⇒ đổi khi Active là **bump `rubric_version`** (như đổi mốc); **tiêu chí bị xoá ⇒ nhãn `targetCriterionIds` của mọi câu trỏ tới nó bị CẮT** (server, cùng transaction, chỉ cắt id chết — câu mà mọi id đều chết còn lại `[]`, câu `null` giữ `null`), audit `EditCriteria` ghi số câu bị cắt |
| POST | `/campaign/{id}/start-now` | **✅ CMP3-B4** Kéo `start_at` về hiện tại (không body). `Employer`. `Draft`/`Closed`/`Archived` → **409** · campaign có ca thi (`campaign_slots`) → **409** (kéo giờ chung vô tác dụng) · **CMP4-B3 — `expires_at` đã qua → **409** (mọi lời mời cũng đã hết hạn; start-now KHÔNG tự dời `expires_at`) · `start_at` đã ở quá khứ → **no-op** (không audit/outbox) · ngoài org → **404**. Hợp lệ → ghi **1 audit `StartEarly`** (mang mốc cũ) + outbox `OpenedEarly` cho lời mời **chưa revoke · chưa hết hạn · `email_sent_at != null`** (CMP4-B3 + **CMP4-B5**: thư "mở sớm" bảo "dùng lại liên kết trong email mời trước đó" — chỉ gửi cho lời mời đã thật sự nhận thư mời). KHÔNG chạm `criteria`/`rubric_version`/`expires_at`. **D32 (2026-09-21): wizard FE KHÔNG gọi endpoint này lúc triển khai nữa** (bỏ ô "Mở ngay khi triển khai"); chỉ trang chi tiết campaign Active dùng |
| PUT | `/campaign/{id}/files` | Thay JD/Criteria (xóa file cũ) |
| PUT | `/campaign/{id}/questions` | Thay toàn bộ câu hỏi. Body `List<QuestionItem>`. Ngoài `Draft` → **409** (CAMP-18). **SC2 · W1 — `targetCriterionIds` BA trạng thái** (mẫu `sampleAnswer`/`levels`, CAMP-16): khoá **VẮNG** ⇒ GIỮ NGUYÊN nhãn đang có (FE cũ không biết field, không được xoá hộ) · `[]` ⇒ XOÁ — lưu **`[]`** ("đã xét, câu không nhắm tiêu chí nội dung nào" ⇒ chỉ chấm `Always`), KHÔNG về `null` · `[ids]` ⇒ THAY (dedup, giữ thứ tự); id ∉ `campaign_criteria` hiện tại ⇒ **400 nêu id**, 0 row ghi. Câu MỚI: vắng ⇒ `null` (chưa gắn) |
| POST | `/campaign/{id}/questions/generate` | **✅ F9 (FR11)** AI sinh câu hỏi từ **JD của campaign** → lưu `source=AiGenerated`, trả `CampaignResponse`. Query `?count=` (1..20; bỏ trống = mặc định AIService). **Thay lượt AI trước đó, GIỮ câu `CustomHr` HR gõ tay** (bấm nhiều lần không cộng dồn). Gọi AIService `POST /api/v1/generate-questions` (JD là **DỮ LIỆU** — AIService bọc delimiter chống prompt-injection, AI-4; AIService không ghi DB, GEN-4). **400** chưa có `jdText` / JD > 20.000 ký tự (CAMP-5) / `count` ngoài 1..20 — guard **TRƯỚC** khi tốn 1 lời gọi AI · **404** ngoài org · **409** campaign không ở `Draft` (CAMP-2) · **502** AIService lỗi hoặc không sinh được câu nào (đề đang có **KHÔNG** bị xoá). **SC2 · W2 — gắn nhãn khi sinh:** payload thêm khoá `criteria: [{criterionId,name,description?}]` = CHỈ tiêu chí `WhenTargeted` (0 tiêu chí ⇒ KHÔNG gửi khoá — prompt nguyên xi), giữ `criteriaContext` đủ bộ; đọc `targetCriteria: string[][]` song song `questions[]` → `questions[].targetCriterionIds` (id ∉ tập WhenTargeted đã gửi ⇒ bỏ + log — I1; `[]` giữ `[]`; vắng khoá ⇒ `null`; **lệch độ dài ⇒ bỏ nhãn CẢ lượt + LogWarning, không 500**) |
...
`QuestionItem`: `{ id?: uuid, questionText, source: "AiGenerated"|"CustomHr" (server sở hữu — F10, giá trị client bỏ qua), isRequired: bool = true, sampleAnswer?: string (ba trạng thái: vắng = không đổi · `""` = xoá · text = ghi đè), questionGroup?: string (nhóm chủ đề — chỉ có nghĩa khi bật ngân hàng đề; null/`""` = nhóm "Chung"), targetCriterionIds?: uuid[] (SC2 · W1 — ba trạng thái: vắng = không đổi · `[]` = xoá (lưu `[]`) · `[ids]` = thay; id lạ → 400) }`. `id` echo lại để GIỮ `source`/`created_at` khi HR sửa (F10).
`CriterionItem` 🔜: `{ id?: uuid, name: string, description?: string, weight: decimal(5,4), maxScore: int, minPct?: int (RNK1 · HĐ-5 — điểm sàn %), scoringScope?: "Always"|"WhenTargeted" (SC2 · W1 — vắng ⇒ Always; lạ → 400; VÀO vân tay), levels?: CriterionLevelItem[] }` — tiêu chí **CÓ CẤU TRÚC** HR nhập thẳng (Σweight=1).
`CampaignResponse.questions[].targetCriterionIds` (SC2): **LUÔN có mặt** — `null` = chưa gắn nhãn (Interview chấm đủ bộ) · `[]` = câu xã giao (chỉ `Always`) · `[ids]` = nhắm. FE echo NGUYÊN khi PUT để không đổi trạng thái (`null` ≠ `[]` — I2). `CampaignResponse.criteria[].scoringScope` (SC2): hàng cũ = `"Always"`.

**RNK1 · HĐ-8 — Ngân hàng đề (`questions_per_session` = K):**
- `POST`/`PUT /campaign` nhận `questionsPerSession?: int`. **Create**: `1..20` hoặc bỏ trống (= thi hết bộ); ngoài dải → **400**. **PUT**: `null`/vắng = KHÔNG ĐỔI · `0` = RESET về "thi hết bộ" (SET NULL) · `1..20` = đặt K. Đổi K ngoài `Draft` → **409** (cùng luật `PUT /questions`: đổi số câu mỗi ứng viên gặp = đổi đề).
- `CampaignResponse.questionBank` (read-time, mọi response): `{ total, alwaysAsked (số câu `isRequired`), questionsPerSession, groups: [{name, count}] (nhóm null/`""` → "Chung", gộp không phân biệt hoa/thường; casing hiển thị = câu sớm nhất của nhóm), warnings: string[], coverageWarnings: [{criterionId, name}] }`. Cảnh báo: K > total · alwaysAsked > K · K×(1+d) > T (ngân sách adaptive, dùng chung `AdaptiveBudgetRule` — HĐ-7) · **SC2 · W1 `K_BELOW_CRITERIA_GROUPS: …`** khi `N = |nhãn[0] câu KHÔNG bắt buộc ∖ nhãn[0] câu bắt buộc| > 0 && K − |required| < N` (BUG-2b: câu bắt buộc chiếm khe + phủ tiêu chí; K null, 0 câu gắn nhãn hoặc N = 0 ⇒ không bắn; thông điệp nêu K, R, N) — vào `warnings` ⇒ **chặn publish** (D-5). **`coverageWarnings`** (SC2): tiêu chí `WhenTargeted` **không câu nào nhắm** (mọi vị trí nhãn; `Always` không bao giờ vào) — **chỉ cảnh báo, KHÔNG chặn publish**, luôn có mặt (`[]` khi sạch), kể cả trên danh sách campaign (nạp 1 projection WhenTargeted cho cả trang).
- **Publish**: `warnings` KHÔNG rỗng → **400** `{ code: "QUESTION_BANK_INVALID", warnings }`. *(Ca K×(1+d) > T đã bị HĐ-7 ném `ADAPTIVE_BUDGET_TOO_SMALL` trước, cụ thể hơn.)*
- **KHÔNG 400 ở save vì K > số câu**: FE gửi `PUT /campaign` (đặt K) TRƯỚC `PUT /questions` (nạp câu) là luồng bình thường — chỉ cảnh báo, gate cứng ở publish. `QuestionPoolSelector` rơi về "thi trọn bộ" khi K ≥ số câu.
- **Rút đề CHO MỖI ỨNG VIÊN** (`ParticipationService` → `QuestionPoolSelector`, deterministic theo `(campaignId, candidateId)`): giữ hết câu `isRequired` → **rổ = tiêu chí CHÍNH** (`targetCriterionIds[0]`, SC2 · W2 — đúng đại lượng INT-18 dùng để chấm), câu chưa gắn nhãn (`null`/`[]`) rơi về `questionGroup` như trước (rổ Guid và rổ tên là hai không gian khoá, không trộn) → **BUG-2 (2026-09-13): B1 mỗi rổ TIÊU CHÍ chưa được câu bắt buộc phủ nhận 1 khe trước** (rổ không nhãn không được ưu tiên — trước đó rổ `""` sort đầu nuốt khe ⇒ tiêu chí bị bỏ cho MỌI ứng viên dù K-rule im) → **B2 water-fill** khe còn lại (rổ quota nhỏ nhất còn dư câu; ≡ chia đều cũ khi khe ≥ rổ; rổ cạn câu ⇒ cân hơn cũ, cùng tổng) → xáo. Selector KHÔNG BAO GIỜ vượt K. Interview nhận đúng K câu (seed = K). `questionGroup` KHÔNG gửi sang Interview (không chấm theo nhóm).
...
- `POST /campaign/{id}/candidates/invite` — **Đường 2 (từ shortlist, 🔜 C15)**: body `{ candidateIds: uuid[], includeIneligible?: bool }` (HR **chọn top** sau ranking) → mỗi candidate: **tách email từ CV** (`campaign_candidates.email` — parse sẵn ở C13) → tạo invitation **gắn `campaign_candidate_id`** + gửi; status `Analyzed → Invited`. **Email null** (CV không có / parse không ra) → **skip item + trả trong `failed[]`** ("thiếu email — PATCH bổ sung"), các item còn lại vẫn gửi bình thường. **RNK1 · HĐ-6:** ứng viên `eligible = false` (thiếu bằng chứng cho nhu cầu must-have) → **skip + `failed[]`** ("Không đủ điều kiện loại…") trừ khi `includeIneligible = true`.
...
- **✅ E11c (2026-09-11) — lịch sử điều chỉnh của HR + nghe ghi âm + transcript đủ field** (`CampaignController`, `Roles=Employer`, org-scoped như override/transcript: ngoài org / session chưa có ranking → **404**):
  - `PUT /campaign/{id}/results/{sessionId}/override` (E11b) nay ghi thêm **1 dòng `ranking_overrides`** mỗi lần Set/Clear, **cùng SaveChanges** với ranking + `audit_logs` (nguyên tử); `actor_email` = snapshot claim `email` của JWT (GEN-3 — không tra Auth lúc chạy), thiếu claim → null.
  - `GET /campaign/{id}/results/{sessionId}/override-history` → `{ sessionId, items[] }` **mới-nhất-trước**, mỗi item `{ id, kind:"Set"|"Clear", score, result, note, actorUserId, actorEmail, at, source:"Live"|"AuditBackfill" }`. `actorEmail = null` = **không biết** (dòng backfill từ audit) — FE hiện "không rõ", không đoán (BK23). Chưa lần nào → `items: []` (không 404).
  - `GET /campaign/{id}/results/{sessionId}/answers/{answerId}/audio` → stream bản ghi âm (proxy Interview `/internal/sessions/{sid}/answers/{aid}/audio`, `Content-Type` theo định dạng thật webm/m4a/wav…). Object key SeaweedFS **không bao giờ ra ngoài** (GEN-5). Answer lạ / chưa có audio → **404**; Interview lỗi → **502**. Không range (FE tải blob).
  - `GET …/transcript` mỗi question **+ additive**: `answerId` · `kind` (Seed|FollowUp|Clarify|NewQuestion) · `answerStatus` · `rejectReason` (`no_speech` = im lặng — CAMP-21; null = không biết) · `durationSec` · `hasAudio` · `sampleAnswer` (F13) · `deliveryMetrics` (F11: `speechRateWpm/pauseCount/longestPauseSec/silenceRatio/fillerCount/fillerBreakdown`, **null = chưa đo, KHÁC 0**) · `scores[].levelMatched` (E9). Trước đó Interview đã trả nhưng client vứt khi map; nay khoá bằng test hợp đồng chéo (JSON thật + đọc thẳng file DTO Interview) — lớp bug "khoá JSON lệch ⇒ field rụng im lặng" đã cắn repo 4 lần.
...
### Phân tích tuyển dụng theo TỔ CHỨC (employer) — ✅ `GET /campaign/analytics`
`GET /api/v1/campaign/analytics?from=<ISO>&to=<ISO>&groupBy=day|month` — `CampaignAnalyticsController` (file riêng, cùng prefix `[Route("campaign")]`), `Roles=Employer` (**cả OrgAdmin lẫn HrMember** — đọc, không phải billing nên AUTH-6 không chặn), `org_id` từ claim JWT (AUTH-8), thiếu → **403**. Bản **ORG-SCOPED** của `GET /campaign/admin/analytics` (FR18, platform-wide, giữ nguyên). Kỳ qua `AnalyticsPeriod.TryResolve` (mặc định 30 ngày gần nhất, nửa mở `[from,to)`): `from >= to` → **400** "`from` phải nhỏ hơn `to`."; `groupBy` lạ → **400** "`groupBy` chỉ nhận `day` hoặc `month`.". **0 migration** (chỉ đọc).
- ⚠ **Route literal `campaign/analytics` đứng cạnh `[HttpGet("{id}")]` của `CampaignController`** — ASP.NET Core xếp segment literal ưu tiên hơn segment tham số (`/Products/List` thắng `/Products/{id}`) nên KHÔNG rơi vào `GetCampaignById`; có test khoá attribute bằng reflection, **L3 phải verify thật trên dev** (`GET /api/v1/campaign/analytics` → 200 JSON analytics, không phải 404 "Campaign analytics not found").
- **Stock vs flow:** `campaigns/screening/invitations/interviews/perCampaign` = trạng thái **HIỆN TẠI** của cả org (KHÔNG lọc theo kỳ); chỉ `buckets` là dòng chảy trong `[from,to)`. Org chưa có campaign ⇒ 200 toàn số 0, mảng rỗng, median `null`. Campaign soft-delete bị loại bởi query filter DB13 (cả 6 truy vấn, kể cả bảng con).
- **Response** (camelCase): `{ from, to, granularity, campaigns:{ total, byStatus[{status,count}] }, screening:{ submissions, analyzed (Analyzed+Invited), byStatus[], medianFitScore, fitDistribution[5 band], riskBySeverity[Low·Medium·High], topSkills[{skill,count}] top 10 }, invitations:{ total, queued, sent, joined, expired, revoked }, interviews:{ joined, started, inProgress, completed, scored, pendingScore, passed, failed, undetermined, medianScore, scoreDistribution[5 band], flagsBySignal[{signalType,count}] }, buckets[{ periodStart, campaignsCreated, invitationsSent, joins, interviewsStarted, scored }], perCampaign[{ campaignId, title, status, createdAt, invited, joined, started, scored, passed, medianScore }] }`. Band `0-19 · 20-39 · 40-59 · 60-79 · 80-100` (biên dưới bao gồm, `100` vào band cuối) và 3 risk **luôn trả đủ** kể cả count 0; median lẻ = phần tử giữa, chẵn = TB 2 phần tử giữa, làm tròn 2 chữ số, rỗng = `null` (KHÔNG phải 0). `topSkills`: trim, so KHÔNG phân biệt hoa/thường, hiển thị dạng gặp đầu tiên, **mỗi CV đếm một kỹ năng tối đa 1 lần**, hoà ⇒ tên A→Z. `pendingScore` = membership có `session_id` ∧ `interview_status=Completed` ∧ `session_id` ∉ `campaign_rankings`.
- **Mốc bucket:** `campaigns.created_at` · `campaign_invitations.email_sent_at` (**KHÔNG** `created_at` — "đã gửi") · `campaign_membership.joined_at` · `campaign_membership.interview_started_at` · `campaign_rankings.updated_at`; sắp tăng theo `periodStart`, bucket rỗng không điền.
- 🔑 **MỘT nguồn sự thật cho luật — `CampaignResultRules`** (`Services/`, `internal static`): `ResolveResult` (override > rớt sàn `ComputeBelowCutoff` > `pass_score_pct`; ngưỡng null ⇒ `null`) · `ResolveDeliveryStatus` (Revoked > Joined > Expired > Sent > Queued) · `InvitationJoinIndex` (ghép theo `membership.invitation_id`, fallback `cv_submission_id`/email CHỈ cho membership lịch sử không có link — FX1). `GetCampaignResultsAsync`/`GetInvitationsAsync` VÀ analytics đều gọi vào đây; test khoá `passed/failed/undetermined` == đếm từ `GetCampaignResultsAsync` và `invitations.*` == đếm từ `GetInvitationsAsync` trên cùng dữ liệu — **đừng chép công thức ra chỗ thứ ba**.
- **Hiệu năng:** 6 truy vấn cố định (campaigns+criteria · cv_submission · invitations · membership · rankings · flags), lọc `org_id`/`campaign_id IN (...)` TRONG SQL, `AsNoTracking` + `Select` đúng cột (không nạp `cv_parsed_text`/`token_hash`), gộp trong bộ nhớ (jsonb `skills`/`scoring_inputs` phải nạp về mới đếm được — GroupBy jsonb không dịch được trên Npgsql lẫn SQLite). Có test khoá số truy vấn không tăng theo số campaign.
...
| Method | Path | Mô tả |
|---|---|---|
| POST | `/campaign/{id}/candidates` | **🔜 C13** Upload **nhiều PDF** CV (`multipart`: `files[]`, mỗi file ≤ 10MB). Parse → `cv_parsed_text`; chạy **hard-filter** (rule cứng) → mỗi ứng viên `Rejected(reason)`/`Filtered`; mỗi `Filtered` → đẩy job AI lên queue. Cần campaign `Active` + đã có `campaign_criteria`; **cap số CV/campaign** (chặn đốt AI vì free) → vượt **4xx** **2026-09-21:** text trích từ PDF được lọc `\0`/ký tự điều khiển ở bộ trích DÙNG CHUNG (`Isas.Shared/Files/ExtractedTextSanitizer`, phủ cả B2C) — trước đó một glyph không bảng Unicode làm Postgres `22021` từ chối INSERT và **cả lô CV đổ theo**; nay DB từ chối một dòng thì chỉ dòng đó thành `Rejected` "CV không đọc được — upload lại." (giữ `cv_file_url`, bỏ text/email đã parse), các dòng còn lại + `job_needs` vừa rút vẫn lưu, audit ghi kèm số dòng bị từ chối. |
| GET | `/campaign/{id}/candidates` | ✅ **C14 + keyset paging** Shortlist. Query `?status=&minScore=&skill=&search=&sort=score\|name&cursor=&limit=`; mặc định `sort=score` DESC (`overall_match_score`). **`status`/`minScore`/`search`/`sort` đều đẩy xuống SQL** (trước đây nạp toàn bộ ứng viên của campaign rồi lọc/sắp trong C#, mà `max_candidates` nullable = có thể KHÔNG có trần). `search` khớp `full_name` **hoặc** `email`, case-insensitive. Lọc theo `org_id` (chủ campaign) → ngoài org = 404. ⚠ **`skill` lọc SAU phân trang** (jsonb `string[]`, không push SQL portable Npgsql↔SQLite) ⇒ trang có thể **ngắn hơn `limit` hoặc rỗng mà vẫn còn trang sau** — client PHẢI đi theo `X-Next-Cursor` tới khi header vắng, không dừng khi thấy trang ngắn. **RNK1 · HĐ-6:** mỗi dòng mang `eligible`/`mustHaveMet`/`mustHaveTotal` (điều kiện loại, đánh giá read-time); sort mặc định `eligible desc, rồi điểm` nhưng **trong phạm vi trang** (eligible không phải cột → không vào ORDER BY/keyset DB; cùng giới hạn `skill`) |
| GET | `/campaign/{id}/candidates/{candidateId}` | ✅ Chi tiết 1 ứng viên: `fitSummary` · `strengths[]`/`gaps[]` (`{needId,area,level,evidence}` — `evidence` là **đoạn TRÍCH từ CV**) · `bonusSignals[]` · `verificationRisk` · `verifyQuestions[≤3]` · `screeningVersion` + **`cvFileUrl`** (S3 KEY). Nhóm `gaps` = việc cần hỏi ở vòng phỏng vấn. **RNK1 · HĐ-6:** `eligible`/`mustHaveMet`/`mustHaveTotal` + `missingMustHave[]` (text nhu cầu must-have chưa đạt) |
| PUT | `/campaign/{id}/job-needs` | ✅ HR chốt **nhu cầu công việc** dùng để sàng CV (replace-all). Body `[{needId?,category,text,isMustHave?}]`, `category ∈ Technical·WorkStyle·Communication·Growth`. `needId` echo lại để kết quả sàng đã lưu còn trỏ đúng dòng (mẫu F10); **`source` KHÔNG nhận từ client** — server sở hữu. **RNK1 · HĐ-6 — `isMustHave` GIỮ giá trị client** (khác `source`: quyết định nghiệp vụ, không phải nhãn nguồn gốc); AI không đề xuất (`false`). **CMP4-B1 — cửa sửa đo CÙNG THƯỚC với luật-lọc-cứng (`PUT /campaign`): campaign đã có BẤT KỲ `cv_submission` nào (Filtered/Analyzing/Analyzed/…) HOẶC `Closed`/`Archived` → **409** (job_needs KHÔNG mang nhãn phiên bản ⇒ đổi thước sau khi có người bị đo là không so sánh được). Gửi `[]` (wipe) khi đã có `cv_submission` cũng **409** — `[]` đi qua đúng cửa đó, không có đường tắt; wipe khi CHƯA có ai → 200. Đổi `isMustHave` sau khi có `cv_submission` cũng 409. `category` lạ → **400** |
...
| domain | varchar(100)? | ⚠ **DƯ — target: bỏ.** Free-text (AI sinh câu hỏi dùng `jd_text`; phân loại role dùng `job_category` 🔜 dưới). ⚠ NHƯNG nhánh sàng CV B2B ĐANG đọc: `BuildCriteriaAsync`/`BuildJobNeedsAsync` (`jobCategory` từ `domain`), `CvScreeningJob.domain`, `POST /questions/generate`. **CMP4-B2** — vì thế `domain` (như `language`) bị khoá bởi cùng thước với 3 luật lọc cứng: có `cv_submission` NÀO / `Closed`/`Archived` → `PUT /campaign` **409** |
...
| job_needs | jsonb? | `[{needId,category,text,source,isMustHave}]` — **nhu cầu công việc dùng để SÀNG CV** (khác `campaign_criteria` = thước chấm buổi phỏng vấn). AI suy từ `jd_text` lúc publish → HR sửa qua `PUT /job-needs` **khi campaign CHƯA có `cv_submission` nào** (CMP4-B1 — cùng thước với luật-lọc-cứng; đã có ứng viên hoặc `Closed`/`Archived` → 409). Chốt MỘT LẦN cho cả campaign để mọi ứng viên đo cùng thước (mẫu CAMP-10). `source` = `AiSuggested`/`HrEdited`, **server sở hữu** (F10). **RNK1 · HĐ-6 — `isMustHave`** (bool, vắng khoá jsonb ⇒ `false`, KHÔNG migration): điều kiện LOẠI — thiếu bằng chứng Strong/Partial cho bất kỳ must-have ⇒ `eligible = false` read-time; HR sở hữu (client giữ giá trị), AI không đề xuất. `must_have_total`/`must_have_met` đếm CHỈ need `isMustHave` (nguồn duy nhất `CvMustHaveEvaluator`). null/rỗng ⇒ **chưa sàng CV được** |
...
| time_limit_minutes | int? | 🔸 **TẠM BỎ — không enforce (áp cả B2B & B2C)**: KHÔNG giới hạn tổng buổi, **chỉ giới hạn TỪNG CÂU** (`practice_questions.time_limit_sec`, mặc định engine — `campaign_questions.time_limit_seconds` đã bị drop). Giữ cột nullable cho tương thích API/schema nhưng **bỏ qua khi xử lý**; muốn bật lại tổng buổi thì enforce cột này + khôi phục trigger auto-submit ở [interview.md](interview.md) §State machine — Session |
| questions_per_session | int? | ✅ **RNK1 · HĐ-8** — K = số câu mỗi ứng viên thi. `null` = thi HẾT bộ (mặc định, chiến dịch cũ). `1..20` (CHECK `ck_campaigns_questions_per_session_positive` chỉ ép `>= 1`; trần 20 ở code — F2b). PUT `0` = RESET về null. Đổi K ngoài `Draft` → 409. `QuestionPoolSelector` rút ĐỀU theo tiêu chí chính `target_criterion_ids[0]` (SC2 · W2), câu chưa gắn nhãn theo `question_group`; giữ hết câu `is_required` |
...
| skip_penalty | bool | ✅ **RNK1 · HĐ-2 / CAMP-21** (migration `AddCampaignSkipPenaltyRnk1`) — NOT NULL DEFAULT true, **SERVER SỞ HỮU** (KHÔNG trên `POST`/`PUT /campaign`; có trên `CampaignResponse` chỉ để hiển thị). Campaign tạo TỪ RNK1 = true; campaign trước RNK1 = false (migration `UPDATE campaigns SET skip_penalty = false`). Gửi sang Interview lúc tạo buổi (`skipPenalty`) → ghim `practice_sessions.skip_penalty`. `true` ⇒ điểm tổng B2B = `clamp(expr × seed_completeness, 0, 100)`. Mẫu `5c900002` "Phạt bỏ câu" bị RÚT khỏi `GET /campaign/scoring-policy-templates` (row giữ lại — `RetiredTemplateIds`) |
...
### `campaign_questions`
`id` · `campaign_id` (FK Cascade) · `org_id` (denormalize theo campaign = ORG, BK4) · `question_text` · `source` (enum `AiGenerated`/`CustomHr` — provenance do SERVER sở hữu, F10; hiển thị nguồn khi HR duyệt) · `is_required` (mặc định true) · `hr_edited_at?` (R10 — mốc HR sửa nội dung câu AI; `source` KHÔNG đổi) · `sample_answer?` (đáp án mẫu HR soạn — ví dụ để AI hiệu chỉnh thang điểm, KHÔNG phải đáp án đúng duy nhất) · `question_group?` (varchar(100) — nhóm chủ đề, RNK1 · HĐ-8) · **`target_criterion_ids?` (jsonb `uuid[]`, SC2 · W1, migration `AddScoringScopeAndQuestionTargetsSc2` — id `campaign_criteria` câu nhắm tới; ⚠ `null` ≠ `[]`: `null` = chưa gắn nhãn ⇒ chấm đủ bộ · `[]` = đã xét, không nhắm ⇒ chỉ tiêu chí `Always`; hàng cũ `null`, KHÔNG backfill, KHÔNG default DB)** · `created_at`.
> ✅ **RNK1 · HĐ-8 — `is_required` NAY CÓ TÁC DỤNG** (qua ngân hàng đề, không qua engine): `QuestionPoolSelector` giữ HẾT câu `is_required` trong mỗi buổi, chỉ rút ngẫu nhiên phần khe còn lại. `practice_questions` (engine) vẫn không có cột này và không cần — mọi câu ĐÃ RÚT đều bắt buộc trả lời (luật đóng session không đổi). Ghi chú cũ "đứt dây / khuyến nghị bỏ cột" **không còn đúng**.
> 🔸 **`time_limit_seconds` ĐÃ BỎ** (migration `DropDeadCampaignColumns`) — tổng buổi không enforce, giới hạn/câu dùng mặc định `practice_questions.time_limit_sec`.
...
- **SC2 · W5/W1 (2026-09-13):** chép **`scoringScope`** từ bộ chuẩn (hợp đồng W5: Interview trả `criteria[].scoringScope`; vắng/lạ ⇒ `Always` + warning — chuẩn hoá ở `CampaignSessionClient`, đường chép tin thẳng). Bộ mới **mint id tiêu chí mới** ⇒ **nhãn `target_criterion_ids` của MỌI câu về `null`** ("chưa gắn" — HR chưa xét gì trên bộ mới; KHÔNG về `[]`), cùng SaveChanges; ghi **audit `ClearQuestionTargets`** (giá trị mới của `ck_audit_logs_action`) kèm số câu — chỉ khi thật sự có nhãn bị xoá.
...
**SC2 · T6 (2026-09-13):** quota `FreeRunsPerQuestion = 1` theo `(campaign_id, rubric_version, question_id)` (trước: 3/(campaign, version)); `question_id` LUÔN resolved (không gửi ⇒ câu đầu theo `created_at, id`); `rubric_snapshot[]` += `scoringScope`, `inScope` (bool? — row cũ null ⇒ toàn bộ); response run += `scopedCriterionIds` (= `Always` ∪ nhãn câu) và `freeRunsRemaining` theo đúng `(version, câu)` của TỪNG run trong `GET` lịch sử; `samples[]` chỉ tiêu chí trong phạm vi, `expected/actualWeightedPct` chia Σweight tập gửi (mẫu số ACTUAL chỉ tiêu chí AI có điểm — mirror `SessionScoringNotifier`). Guard ≥2 mốc chỉ đòi trên phạm vi; câu `[]` mà 0 tiêu chí `Always` ⇒ 400 trước reserve.
...
| scoring_scope | varchar(16) | ✅ **SC2 · W1 (2026-09-13, migration `AddScoringScopeAndQuestionTargetsSc2`)** — `Always` (chấm MỌI câu — cách nói) · `WhenTargeted` (CHỈ khi `campaign_questions.target_criterion_ids` nhắm tới — nội dung; câu không nhắm ⇒ LOẠI khỏi điểm câu đó, không tính 0 — INT-18). **NOT NULL DEFAULT `'Always'`** + CHECK `ck_campaign_criteria_scoring_scope` (danh sách đóng). Hàng cũ = `Always` qua DEFAULT (hành vi trước SC2), KHÔNG backfill. **VÀO vân tay** `RubricFingerprint` (khoá `Scope` sau `MaxScore`) ⇒ đổi khi Active = bump `rubric_version`. Gửi sang Interview lúc tạo session (W4) |
| created_at / updated_at | timestamptz | `now()` |

→ Khi tạo session, gửi sang Interview để materialize thành `rubric_criteria(campaign_id)` (id phía Interview **khác** id ở đây — ref lỏng, copy giá trị `name`/`weight`/`max_score`/`scoring_scope`; `rubric_criteria.source_criterion_id` giữ id bên này để map `target_criterion_ids`).

**🔜 Lưu structured `criteria[]` — CẨN THẬN:**
- **Khi nào ghi:** create/update lúc campaign còn `Draft`; **`Active` cũng sửa được tiêu chí + mốc + scope** (ngoại lệ có chủ đích với CAMP-2, an toàn nhờ `rubric_version` — CAMP-18); `Closed`/`Archived` → **409**.
- **Merge theo id, KHÔNG "xoá hết rồi insert lại"** (RNK1 · HĐ-5, `BuildStructuredCriteria` mutate tại chỗ): tiêu chí **echo `id`** ⇒ UPDATE tại chỗ, **id GIỮ NGUYÊN** (khoá ổn định cho snapshot chấm `criterionId` + nhãn `target_criterion_ids`; ⚠ **BUG-1 (vá 2026-09-13):** mốc mới của tiêu chí tái dùng đến EF qua nav fixup với `Id` gán sẵn + khoá `gen_random_uuid()` ⇒ bị track **Modified** ⇒ `UPDATE … 0 row` ⇒ 500 `DbUpdateConcurrencyException` cho MỌI PUT echo id có mốc từ RNK1 03/09 tới 13/09 — nay `MarkNewLevelsAdded` đánh `Added`; test SQL-shape khoá INSERT-không-UPDATE); tiêu chí KHÔNG được echo ⇒ xoá (FK Cascade kéo mốc đi theo; **SC2: nhãn câu hỏi trỏ tới nó bị CẮT** cùng transaction — chỉ id chết, id sống giữ); item không `id`/id lạ ⇒ INSERT id mới. `order_no` đánh lại theo thứ tự gửi lên. Vẫn **1 transaction** (1 SaveChanges). *(Câu cũ "xoá hết rồi insert lại" lệch code từ RNK1 — đã sửa 2026-09-13.)*
...
> Tạo/reissue invitation → ghi outbox-row **CÙNG transaction** (thay publish best-effort SAU commit) → `OutboxDispatcher` (BackgroundService, `Outbox:*`) publish `campaign_invitation_email_queue` at-least-once (broker down → giữ null → gửi lại). Consumer `InvitationEmailConsumer` dedup **nhánh thư mời** theo `campaign_invitations.email_sent_at` (bền vững). **CMP4-B5 — nhánh `OpenedEarly`** (thư "mở sớm", CMP3-B4) dedup + trần thất bại RIÊNG, keyed by **messageId của outbox row** (`= BasicProperties.MessageId`): (a) dedup **trong tiến trình** (`ConcurrentDictionary` TTL 6h — route A, KHÔNG cột; cửa hở duy nhất = consumer restart trong cửa sổ redeliver → tối đa 1 thư trùng, vô hại) · (b) consumer guard `email_sent_at != null` (đồng bộ producer filter) · (c) sender ném ≥ N lần (`OpenedEarlyEmail:MaxAttempts`, mặc định 5) → **THÔI requeue** + log Error (rơi khỏi hàng đợi thay vì poison-loop chặn cả hàng đợi thư mời — queue chưa có DLX). **Nhánh thư mời KHÔNG đụng** (vẫn throw → caller requeue). Mirror DB2 (Interview). *(cv_screening dùng reconciler-theo-state riêng — `last_screening_published_at` + `StuckScreeningRepublisher`, không qua bảng này.)*
...
### `ranking_overrides` — lịch sử điều chỉnh của HR (E11c, append-only, migration `AddRankingOverridesE11c`)
`id` · `ranking_id` (FK → `campaign_rankings`, Restrict) · `campaign_id` · `session_id` (denorm) · `kind` varchar(8) `Set|Clear` (**không CHECK** — thêm giá trị không được thành cửa một chiều ở `Down()`) · `score` numeric(5,2)? · `result` varchar(10)? · `note` text NOT NULL · `actor_user_id` · `actor_email` varchar(255)? (snapshot claim JWT; **NULL = không biết**) · `source` varchar(16) `Live|AuditBackfill` · `created_at` (= `overridden_at` của ranking, cùng một `now`). Index `(ranking_id, created_at)`. Query filter chuỗi `Ranking.Campaign.DeletedAt == null` (DB13).
- **5 cột `override_*` trên `campaign_rankings` = trạng thái HIỆN TẠI** (ranking sort / CSV / preview vẫn đọc chúng; huỷ = null hết). Bảng này = **trail** — một dòng mỗi lần đặt/huỷ, không bao giờ update/delete. Mục P2 "tách `ranking_overrides`" (tasks.md) đóng tại đây.
- **Backfill** trong migration: parse `audit_logs.summary` (`Override session {sid}: score=…, result=…. Lý do: …` / `Huỷ override session {sid} (về điểm AI). Lý do: …`) từng trường bằng `substring(from pattern)`; score chỉ cast khi khớp `^[0-9]+(\.[0-9]+)?$`; không tìm `'. Lý do: '` ⇒ note = nguyên summary; session không còn ranking ⇒ bỏ (preflight dev 4/4 · prod 22/22 khớp). Postgres-only — không test .NET nào phủ, verify bằng đếm sau apply.
...
| last_screening_published_at | timestamptz? | **CMP4-B4 — mốc BẮT ĐẦU LƯỢT ĐÁNH GIÁ hiện tại** (KHÔNG phải "lần publish gần nhất"): set 1 lần ở đầu lượt (publish batch / rescreen / lần `Filtered→Analyzing` đầu của sweeper), KHÔNG dời khi `StuckScreeningRepublisher` đẩy lại (nhịp đẩy-lại đọc `updated_at`). Trần bỏ cuộc neo vào `COALESCE(last_screening_published_at, created_at)`. `null` = `Filtered` chưa từng publish |
| created_at / updated_at | timestamptz | `now()` |
...
`id` · `org_id` · `actor_user_id` · `action` (`CreateCampaign`/`EditQuestions`/`EditCriteria`/`Publish`/`Delete`/`Reissue`/`ScreenCandidates`/**`CreateApiKey`**/**`RevokeApiKey`**/`ApplyScoringPolicy`/`StartEarly`/**`ClearQuestionTargets`** (SC2 — from-system-default xoá nhãn `target_criterion_ids` mọi câu)…) · `entity` · `entity_id` · `summary`/`diff?` · `at`. CHECK `ck_audit_logs_action` là danh sách ĐÓNG — thêm action = migration nới CHECK (khuôn `AddAuditActionStartEarlyCmp3B4`).
...
- **Stuck** (`StuckScreeningRepublisher`, mẫu `StuckAnswerRepublisher`): `Filtered` quá hạn mà `last_screening_published_at=null` (publish hụt) **hoặc** `Analyzing` mà `updated_at` (lần ghi gần nhất — đầu lượt hoặc lần đẩy trước) quá 15' không callback (worker mất tích) → đẩy lại job. Đẩy lại chỉ dời `updated_at`. **Trần bỏ cuộc** (`Screening:GiveUpAfterHours`, mặc định 6h) → thôi đẩy lại + lật `AnalysisFailed` để HR nhìn thấy. **CMP4-B4 — trần neo vào `COALESCE(last_screening_published_at, created_at)` = MỐC BẮT ĐẦU LƯỢT**, KHÔNG phải `created_at` (upload): CMP3-B2 cho `cv_submission` sống nhiều ngày trong Draft ⇒ rescreen một hồ sơ cũ rồi bị lật `AnalysisFailed` sau 15' vì `created_at` đã quá 6h. **CMP4-B1 — kiểm `job_needs` TRƯỚC trần**: campaign chưa chốt nhu cầu công việc là lỗi **cấu hình**, vẫn lật `AnalysisFailed` nhưng `reject_reason` nói "chưa chốt job_needs — sàng CV không chạy được", **KHÔNG** dán nhãn "worker sàng CV không phản hồi".
