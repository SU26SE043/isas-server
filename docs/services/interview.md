...
**`GET /sessions/history`** — Lịch sử của chính user. Res **`200`** `PracticeSessionSummary[]` (mới nhất trước; ✅ BC9: kèm `overallScore` nếu buổi đã `Scored`). **keyset-paged** (`?cursor=&limit=`, header `X-Next-Cursor`). `?status=` (vd `Scored`) + `?excludeCampaign=true` — OPT-IN cho picker buổi luyện của wizard lộ trình; giá trị `status` lạ → **400** (khớp chặt, case-sensitive — KHÔNG fail-open). Lỗi: **400** · **401**.
