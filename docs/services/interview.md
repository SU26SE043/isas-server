...
>
> **INT-17b — chuỗi đào sâu theo từng câu gốc:** `practice_questions` thêm `depth int NOT NULL DEFAULT 0` + `root_question_id uuid NULL` (ref lỏng cùng bảng, **KHÔNG FK** — tránh cascade path phụ, cùng lý do `generated_from_answer_id`) + index `(session_id, root_question_id, depth)`; `practice_sessions` thêm `max_deep_per_question int NOT NULL DEFAULT 0` (**0 = chế độ cũ ⇒ kill-switch chỉnh được lúc chạy, không cần deploy lại**) + `adaptive_failures int NOT NULL DEFAULT 0`. Migration `AddQuestionChainDepthInt17b` (reversible — mọi cột đều là dữ liệu dẫn xuất).
