...
`GET /internal/rubrics/b2c?jobCategory=&language=` (`X-Internal-Token`) — cấp bộ chuẩn cho CampaignService (CAMP-20). **Không trả `id`** (id Interview vô nghĩa với Campaign) và **không trả `scoring_scope`**.
