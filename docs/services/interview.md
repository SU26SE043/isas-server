...
`api/admin/rubrics` (`Roles="Admin"`, qua `interview-admin-route` sẵn có): `GET ?language=` ma trận 3 nghề × trạng thái mốc · `GET {jobCategory}` (kèm **`sampleQuestions[{id,text}]`** cho chấm thử — nội dung câu mẫu chỉ tồn tại **một chỗ**, FE không hardcode) · `PUT` · `DELETE` (reset) · `GET {jobCategory}/history`.
