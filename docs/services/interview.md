...
```
- ⚠ **Fix 2026-09-15:** `focusTrackingEnabled: true` nay phản ánh ĐÚNG ngay trong response 201 (kèm `focusEvents: []`) — trước đó response 201 luôn báo `false`/`null` dù DB đã ghi `true` (đọc từ tham số mặc định thay vì entity), nên FE hydrate store từ response tạo buổi thấy listener không bao giờ bật; `GET /sessions/{sessionId}` ngay sau vẫn luôn đúng.
...
**`GET /sessions/history`** — Lịch sử của chính user. Res **`200`** `PracticeSessionSummary[]` (mới nhất trước; ✅ BC9: kèm `overallScore` nếu buổi đã `Scored`). **keyset-paged** (`?cursor=&limit=`, header `X-Next-Cursor`). `?status=` (vd `Scored`) + `?excludeCampaign=true` — OPT-IN cho picker buổi luyện của wizard lộ trình; giá trị `status` lạ → **400** (khớp chặt, case-sensitive — KHÔNG fail-open). `?source=lesson|free` — OPT-IN, tách buổi sinh từ **bài học lộ trình** (`lesson` = có dòng `roadmap_lesson_attempts`, cùng nguồn với nhãn `lessonTitle`) khỏi buổi **luyện tự do** (`free`); lọc **trong SQL, TRƯỚC khi cắt trang**. Hai giá trị là **PHÂN HOẠCH** của tập chưa lọc (`lesson ∪ free` = đúng tập khi không lọc) và **trực giao** với `excludeCampaign` — buổi B2B thuộc nhóm `free`, muốn "tự do và chỉ B2C" thì ghép `?source=free&excludeCampaign=true`. Cố ý KHÔNG cho `free` ngầm loại B2B: làm vậy thì có buổi không nằm trong tab nào và người dùng bật tab nào cũng không thấy nó. Giá trị lạ → **400**. Lỗi: **400** · **401**.
- `PracticeSessionSummary` = `{ id, status, jobCategory, createdAt, completedAt, overallScore, seniority, lessonTitle }`.
- **`lessonTitle`** = tên BÀI HỌC của lộ trình mà buổi này thuộc về; **`null` = buổi luyện TỰ DO**. `practice_sessions` KHÔNG có cột tên nào, nên trước field này bảng "chọn báo cáo" của wizard chỉ có `jobCategory` để hiển thị ⇒ mọi buổi Backend hiện đúng một chữ "BE" (đo trên dev: 8 buổi `BE|Junior` liên tiếp không phân biệt được). Nguồn: `roadmap_lesson_attempts` (UNIQUE `session_id` ⇒ ghép 1–1) → `roadmap_lessons.title`, **không** qua `roadmap_lessons.session_id` (cột đó chỉ trỏ buổi MỚI NHẤT nên bài đã luyện lại làm buổi cũ mất nhãn). ⚠ `null` không hiếm và không phải lỗi — đo trên dev 3/18 buổi B2C đã chấm là luyện tự do; **client tự ghép nhãn từ nghề + cấp độ + thời điểm, KHÔNG dựng tên rồi trình bày như tên thật**. Không migration (đọc bảng có sẵn).
...
**`PUT /{id}`** (multipart, thay file) → `FileRecord` mới · **`DELETE /{id}`** → **`204`** — ✅ **SOFT-DELETE (2026-09-15)**: đóng dấu `file_records.deleted_at` **TRƯỚC**, rồi mới xoá key S3 best-effort (S3 lỗi chỉ log — object thừa là việc của sweep BK29). File đã xoá **biến mất khỏi mọi đường đọc của người dùng** (`GET /files`, `GET /{id}`, `/download`, `/parsed-text`, và **không dùng được cho buổi/roadmap/phân tích MỚI** — `cvId`/`jdId` trỏ tới file đã xoá xử như không tồn tại), nhưng **row + tham chiếu giữ nguyên**: buổi luyện/roadmap/`cv_analyses` đã dùng nó không đổi, buổi đang chạy dở vẫn đọc CV theo `session.cv_id` (AnswerService không qua bộ lọc). Xoá lần hai → **204** (idempotent). *Vì sao:* 3 FK `ON DELETE RESTRICT` (`practice_sessions.cv_id/jd_id`, `roadmaps.cv_id`) làm CV/JD **đã dùng là không xoá cứng được**, và đường cũ xoá S3 **trước** rồi DB mới từ chối ⇒ đo prod 2026-09-15: **9/49 row "zombie"** (row còn, file mất, tải 500, xoá 500). Migration `AddFileRecordSoftDelete` (1 cột nullable, additive).
- **`GET /{id}/download`** khi row còn mà object S3 đã mất (di sản zombie) → **404** *"File không còn trên hệ thống lưu trữ"* thay vì 500 "thử lại" — lỗi vĩnh viễn, không phải tạm thời.
Lỗi chung Files: **401** · **403** (không phải file của bạn) · **404**.
> 🐛 **Đã sửa:** 5 endpoint trên từng trả **500** thay vì 403 cho file của người khác. Chúng dùng `Forbid("thông điệp")`, mà overload đó là `Forbid(params string[] authenticationSchemes)` ⇒ chuỗi tiếng Việt bị hiểu là **tên authentication scheme**; `ForbidAsync` không tìm thấy handler nào tên vậy nên ném `InvalidOperationException`, và InterviewService không có `UseExceptionHandler` ⇒ 500 kèm stack trace. Với tới được chỉ bằng cách đọc file của người khác, và **không test nào chạm 5 endpoint đó** nên nó sống im lặng. Nay dùng `StatusCode(403, new { error })` như `PracticeController`, kèm test hành vi + một test **quét mã nguồn** cấm `Forbid("…")` ở mọi controller (lỗi lúc chạy, trình biên dịch không kêu).
...
- 🔑 Picker "chọn lộ trình đã hoàn tất" của wizard phải dùng **`hasFinalReport`**, KHÔNG dùng `status=Completed`: `CreateAsync` gác bằng `IsNullOrWhiteSpace(prior.FinalReport)` → 400, còn `RoadmapLessonService.RetryLessonAsync` mở lại roadmap `Completed → Active` và **xoá** `FinalReport` ⇒ hai vị ngữ không đồng nhất theo cả hai chiều. Lọc theo status = mời một lộ trình rồi bắt người dùng ăn 400 SAU KHI đã chờ 13–54s tạo roadmap.
- `RoadmapSummaryResponse` = `{ id, name, jobCategory, level, mode, cvId, status, createdAt, completedAt, hasFinalReport }` — **KHÔNG** có `milestones`. (`mode` có ở list vì đây là nơi cần nhất: ba lộ trình cùng nghề + cùng level nằm cạnh nhau thì "ôn tập" hay "tiến lên" là thứ duy nhất phân biệt được chúng.) Trước đây list `Include(Milestones).ThenInclude(Lessons)` nên payload nhân theo cây cho một màn hình chỉ vẽ tiêu đề/ngày/trạng thái. Cần cây đầy đủ (kèm `theoryContent`) → gọi `GET /roadmaps/{id}`.
...
skip_penalty  bool          ✅ **RNK1 · HĐ-2 / CAMP-21** (migration `AddSessionSkipPenaltyRnk1`) — NOT NULL DEFAULT false. Ghim luật "câu HR khai mà ứng viên bỏ trống tính 0 điểm" của buổi B2B: `true` ⇒ điểm tổng = `clamp(expr × seed_completeness, 0, 100)`. Nhận từ `CreateCampaignSessionInternalRequest.skipPenalty` (= `campaigns.skip_penalty`); campaign trước RNK1 = false. ✅ **2026-09-21: B2C cũng ghim `true`** ở `CreateSessionInternalAsync` (phủ cả lesson) — `SessionResultService` nhân `overall × seed_answered/seed_total` qua cùng `SkipPenaltyRule.Apply`; buổi B2C cũ = false ⇒ không hồi tố
...
### `practice_face_images` — ✅ 2026-09-17 (migration `AddPracticeFaceCheckB2c`) — sổ ảnh webcam B2C (bình thường RỖNG: ảnh xoá ngay sau detect, dòng còn lại = phần sót chờ purger)
```
id           uuid          PK
...
>
> ⚠ **`adaptive_enabled` là NGOẠI LỆ trong bảng trên — gói không tắt được nó (INT-19).** `ResolveSessionSettings` lấy `config.Enabled || (tiering && entitlement.AdaptiveEnabled)`: cờ `Adaptive:Enabled` là **SÀN**, gói chỉ **CỘNG** (bật sớm cho một tier khi rollout chung còn tắt) chứ không **TRỪ**. Lý do: buổi đã trừ 1 credit trước đó ở bước reserve, hạ cấp xuống luồng tĩnh là lấy tiền rồi giao hàng khác — mà hai đường hạ cấp đều **không lỗi, không cảnh báo**: Payment sập (→ `EntitlementSnapshot.Free`) và plan admin tạo quên tick adaptive (DTO mặc định `false`).
> **Trần buổi:** gói chỉ có tiếng nói khi cấp một số **DƯƠNG**; `0` = "gói không khai trần riêng" ⇒ rơi về `Adaptive:MaxQuestions`, KHÔNG phải `Math.Clamp(x, 0, 0)` = 0 câu. Trần dương của gói vẫn thắng và vượt trần vẫn **400 trước reserve** (PAY-5). Khoá bằng `AdaptiveEveryTierTests` — trước nó, `grep Tiering` trong test project Interview ra **rỗng**: cả nhánh tiering chưa từng có test nào chạy qua, trong khi bật nó trên prod chỉ là một biến môi trường.
...
reject_reason             text?         ✅ CAMP-21 (migration AddAnswerRejectReasonCamp21, 2026-09-11) — lý do bản chép bị từ chối; giá trị duy nhất 'no_speech' (VAD xác nhận im lặng), ghi ở ĐÚNG 2 chỗ (AnswerService nhánh thích ứng + callback worker noSpeech=true), KHÔNG ghi ở chốt sổ buổi kẹt. SessionScoringNotifier loại bài có giá trị này khỏi answered/seed_answered; NULL = "không biết" ⇒ vẫn tính (không hồi tố). Không HasMaxLength, không CHECK (tiền lệ transcript_engine). Reset khi upload lại (INT-3).
...
### Bộ chuẩn B2C do ADMIN quản — ✅ **BC-8 (2026-08-13)**, migration `AddB2CRubricPinning` + `AddAdminRubricPreviewRuns`
`api/admin/rubrics` (`Roles="Admin"`, qua `interview-admin-route` sẵn có): `GET ?language=` ma trận 3 nghề × trạng thái mốc · `GET {jobCategory}` (kèm **`sampleQuestions[{id,text}]`** cho chấm thử — nội dung câu mẫu chỉ tồn tại **một chỗ**, FE không hardcode) · `PUT` · `DELETE` (reset) · `GET {jobCategory}/history`. ✅ **2026-09-16 (đợt B): `criteria[].scoringMethod`** (`"Ai"`|`"DeliveryMetrics"`) — `Ai` = LLM chấm, mốc là THƯỚC ĐO; `DeliveryMetrics` = tính từ số đo giọng nói F11 (`DeliveryFluencyScorer`), KHÔNG gửi LLM, mốc chỉ là lời giải nghĩa bậc và **chấm thử không đòi mốc** (`MeasuredCriteriaSplit.ForAi`). Lộ ra vì FE từng chặn cứng chấm thử khi "Độ trôi chảy" 0 mốc — đoán theo TÊN thì sai theo ngôn ngữ. Cùng ngày: ma trận `GET ?language=` và `history` **chỉ đếm tiêu chí `Ai`** vào `criteriaCount`/`withLevelsCount` (bộ chuẩn: mẫu số **6**, không 7) — đếm cả tiêu chí đo là ô ma trận báo "thiếu mốc" vĩnh viễn cho thứ không cần.
Chấm thử admin: bảng `admin_rubric_preview_runs`, quota **5 lượt `Succeeded` / (nghề, ngôn ngữ, version)**, không đụng Payment (admin không có ví). `POST` nhận `sampleQuestionId` **hoặc** `question`; id lạ → **400 nêu id hợp lệ**, không im lặng rơi về câu mặc định.
**Tự thử thước đo (2026-09-16):** admin **nói** vào mic (hoặc dán) rồi xem hệ chấm mình. `POST {jobCategory}/preview/transcribe` (multipart `file`, `?language=vi|en`, trần 15 MB, `Roles=Admin`) → Interview gọi AIService `/transcribe` → `{transcript, deliveryMetrics, transcriptEngine, noSpeech}`; **không tốn lượt, không lưu gì**; VAD không thấy tiếng nói ⇒ 200 `noSpeech=true`. `POST {jobCategory}/preview` nhận thêm **`includeAiSamples`** (mặc định `true` — hợp đồng cũ không đổi; `false` ⇒ KHÔNG sinh 3 bài AI, chỉ chấm `customAnswer`, bắt buộc có ⇒ 400) và **`deliveryMetrics`** (số đo của chính bản ghi, echo từ bước chép lời). Có số đo ⇒ bài `Custom` được chấm với khối F11 thật **và** tiêu chí đo-bằng-máy (trôi chảy) có hàng điểm qua `DeliveryFluencyScorer` (cùng ngưỡng đường chấm thật; nói dưới sàn ⇒ loại, không 0); `AdminPreviewSample.deliveryMetrics` lưu trong jsonb lượt; `deliveryMetricsAvailable` = có bài mang số đo. 3 bài AI và bài dán tay **không bao giờ** có hàng đo. Mẫu số điểm gộp = số tiêu chí thật sự có điểm (INT-10).
`GET /internal/rubrics/b2c?jobCategory=&language=` (`X-Internal-Token`) — cấp bộ chuẩn cho CampaignService (CAMP-20). **Không trả `id`** (id Interview vô nghĩa với Campaign). ✅ **SC2 · W5 (2026-09-13): NAY TRẢ `criteria[].scoringScope`** (`"Always"`|`"WhenTargeted"`) — Campaign chép về `campaign_criteria.scoring_scope` (CAMP-20). *(Câu cũ "không trả scoring_scope" đúng tới trước SC2.)*
> Biên độ quyền admin, luật soft-version, ghim owner, và vì sao backfill phải ba tầng: rules.md **BC-8**. Luật mốc dùng chung `Isas.Shared.Rubric.CriterionLevelRules` với Campaign — hai bản luật lệch nhau nghĩa là admin lưu được nhưng employer **không chép được** (502), mà lỗi nổ ở service khác với chỗ nhập sai.

### Ghim phiên bản thước đo theo BUỔI THI — ✅ **CAMP-18** (B2B) · **BC-8** (B2C), migration `AddSessionCampaignRubricVersion` + `AddB2CRubricPinning`
...
deleted_at     timestamptz?   ✅ 2026-09-15 (migration `AddFileRecordSoftDelete`) — soft-delete; NULL = còn sống. Lọc ở StorageService (đường người dùng), KHÔNG HasQueryFilter toàn cục (buổi đang chạy đọc theo session.cv_id phải vẫn thấy)
```
...
- **✅ 2026-09-15 — buổi kẹt `GeneratingQuestions` (zombie) — vá 2 lớp.** Đo prod: 2 buổi B2C nằm `GeneratingQuestions` từ 28/07 và 20/08 (`updated_at == created_at`), reservation đều đã Released. Gốc: lỗi sinh câu hỏi là **HUỶ request** (client đóng tab / AI timeout → `OperationCanceledException`) ⇒ `ct` đã cancel nên `SaveChangesAsync(ct)` ghi `Failed` ném ngay, `Failed` không bao giờ xuống DB; catch ngoài (P1-2) hoàn credit bằng token không huỷ nên tiền không mất — nhưng session thành zombie: không sweeper nào quét, `OrphanReservationReconciler` coi là in-flight (nếu release cũng hỏng thì credit treo mãi), `EnsureCapacityAsync` đếm nó vào slot. **(1)** `EnqueueGenerationFailedAbandonAsync` ghi `Failed` + outbox `generation_failed` bằng `CancellationToken.None`. **(2)** `SessionAbandonSweeper` thêm pass quét `GeneratingQuestions` có `created_at` quá **`Scoring:GenerationStuckMinutes`** (mặc định **15** — mọi timeout AI ≤ 180s; `0` = tắt) → `Failed` + outbox `SessionAbandoned(generation_failed)` cùng transaction, guard `WHERE status = GeneratingQuestions` để buổi vừa Ready giữa chừng không bị lật (hoàn credit cho buổi đang chạy = buổi miễn phí). 2 zombie prod sẽ tự được chốt sổ sau deploy (Payment release no-op vì đã Released).
...
- **Nguồn tiêu chí tùy mode:** B2C dùng **rubric theo `JobCategory`** (`version` + `is_active`; 1 nghề chung 1 version); **B2B dùng tiêu chí campaign CÓ CẤU TRÚC** — Campaign gửi kèm khi tạo session, Interview materialize thành `rubric_criteria(campaign_id)`. **Pipeline chấm + `answer_scores` giữ NGUYÊN**, chỉ đổi *nguồn tiêu chí* (không chấm trên `criteria_text` thô). **✅ I1:** `PracticeService.CreateCampaignSessionAsync(candidateId, { campaignId, jobCategory, questions[], criteria[] })` → session gắn `campaign_id` + materialize criteria → `rubric_criteria(campaign_id)`, **idempotent theo `campaign_id`** (dùng chung mọi session của campaign). **✅ D2:** HTTP entry = `POST /internal/sessions/campaign` (X-Internal-Token) → `GetOrCreateCampaignSessionAsync` (create-or-get idempotent theo (candidateId,campaignId) chưa-terminal); CampaignService gọi khi ứng viên bấm **Start** (sau khi Join campaign — membership model).
  - **✅ RNK1 · HĐ-8 — `request.Questions` = K câu ĐÃ RÚT** (`campaigns.questions_per_session`, mặc định null = trọn bộ). `ParticipationService` rút CHO MỖI ứng viên qua `QuestionPoolSelector` (deterministic theo `(campaignId, candidateId)` — vào lại nhận đúng đề; giữ hết câu `is_required`, chia khe còn lại theo rổ = tiêu chí chính — SC2/BUG-2, xem campaign.md; xáo). Interview materialize **1:1** thành `practice_questions(kind='Seed')` ⇒ **seed = K**. ✅ **SC2 · W4 (2026-09-13):** request mang `criteria[].scoringScope` (`"Always"`|`"WhenTargeted"`, vắng/null ⇒ `Always`) và `questionDetails[].targetCriterionIds` (id **campaign_criteria**, ba trạng thái `null`/`[]`/`[ids]` — I2) song song `questions[]` (lệch độ dài ⇒ BỎ CẢ details + warning). Interview map id campaign → id rubric qua `rubric_criteria.source_criterion_id` (cả nhánh materialize idempotent, KHÔNG lọc `is_active`; id lạ bỏ + LogWarning; `[]` giữ `[]`) ⇒ `practice_questions.target_criterion_ids` (id RUBRIC); ghi `practice_sessions.scoring_scope_version = 2` khi có ≥1 nhãn `!= null` (trước SC2 B2B luôn `1`). `rubric_criteria.scoring_scope` copy từ `scoringScope`. Từ đó `ScoringScopeFilter` (INT-18) áp cho B2B như B2C: tiêu chí `WhenTargeted` không câu nào nhắm rơi khỏi CẢ tử lẫn mẫu; câu đào sâu thừa kế nhãn cha. `question_group` KHÔNG được gửi (không chấm theo nhóm). **Công bằng CAMP-10 = cùng K + rút đều**, KHÔNG phải "cùng đề".
...
|---|---|---|
| `PeerAverage` | Trung bình % của **người dùng KHÁC** trên chính hệ thống: cùng `job_category` + cùng `language`, buổi **B2C** đã `Scored`, tạo trong `PeerWindowDays` ngày gần nhất | `Trung bình người luyện cùng vị trí (n=N)` |
...
**Sáu quyết định dễ bị "sửa cho gọn", ghi lại lý do** (`CriterionBenchmarkService`, đều có test khoá):
...
5. **Cửa sổ thời gian `PeerWindowDays`.** Mẫu **cố ý** materialize rồi gom trong C# (SQLite của test map `Average(decimal)` qua `ef_avg` lệch Postgres — cùng lý do BC9), nên chi phí bám theo **toàn bộ** lịch sử của mọi người cùng nghề, ngay trên đường nóng nhất (lần poll trả báo cáo khi buổi `Scored`, rồi lặp mỗi lần mở trang Kết quả). Cửa sổ đổi trần đó thành "lưu lượng N ngày", đồng thời làm nhãn đúng nghĩa hơn: so với **người đang luyện gần đây**. `<= 0` = tắt cửa sổ (hành vi cũ). ⚠ **Phòng xa, không phải chữa sự cố** — bảng hiện mới vài trăm dòng, đây **không** là nguyên nhân của bất kỳ độ trễ nào đang đo được.
6. **Cache dùng chung, khoá KHÔNG có `candidate_id`.** Mốc giống nhau cho mọi người cùng `(job_category, language, BỘ tên tiêu chí)`; nhét candidate vào khoá thì cache thành per-user và mỗi người dùng mới vẫn trả nguyên giá một lượt quét. Nên cache **tổng của cả cộng đồng** `(Σ%, số dòng, số buổi)` rồi **trừ phần của chính người xem** (truy vấn thứ hai rẻ, đã có `ix_practice_sessions_candidate_history`). Ảnh chụp là một thời điểm còn vế "của mình" đọc hiện tại ⇒ có thể **trừ hơi nhiều**, nhưng bảng này chỉ được THÊM nên sai lệch **chỉ đi theo hướng an toàn**: chính mình **không bao giờ** lọt vào mẫu (quyết định 1 nguyên vẹn). Trừ ra `<= 0` ⇒ coi như không đủ mẫu → ngưỡng nội bộ.

**Read-time, không snapshot vào bảng** (mốc đổi theo dữ liệu cộng đồng nên bản lưu sẽ lỗi thời ngay mà không ai chịu trách nhiệm làm mới). Dựng trong `GET /practice/sessions/{id}` (cùng chỗ BC9/BC8). **Một migration duy nhất**, chỉ thêm index — không cột, không bảng: `AddPeerBenchmarkIndex` → `ix_practice_sessions_peer_benchmark` trên `practice_sessions (job_category, language, created_at)` filter `campaign_id IS NULL AND status = 'Scored'` (partial: hai vế so với HẰNG nên planner chứng minh được — cùng lập luận đã verify ở `ix_practice_sessions_deadline`). Phía `session_criterion_scores` **không cần** index mới: nối bằng `session_id`, đã là tiền tố trái của `ix_session_criterion_scores_session_id_criterion_id`.

**Config `Benchmark`:** `Enabled` (mặc định `true`; `false` → `benchmark = null` → radar về 1 lớp như trước F14) · `MinSampleSize` (mặc định **5** buổi của người khác **cho mỗi tiêu chí**) · `PeerWindowDays` (mặc định **90**; `<= 0` = tắt cửa sổ) · `CacheTtlSeconds` (mặc định **300**; `<= 0` = tắt cache).

> ⚠ **DI:** `CriterionBenchmarkService` nhận `IMemoryCache` ⇒ `Program.cs` của InterviewService phải có `builder.Services.AddMemoryCache();` (mẫu đã dùng ở Auth/Campaign; **không** thêm package — nằm sẵn trong framework ASP.NET Core). Thiếu dòng này thì host **fail ngay lúc khởi động** ở Development (`ValidateOnBuild`), không phải hỏng lặng lẽ.
...
**Shape event:**
- `SessionScored` = `{ sessionId, campaignId?(null=B2C), candidateId, totalScore, scoredAt }` (+ SCP1/RNK1: `rubricVersion?`, `scoringInputs?`, `scoreFallback`, `campaignPolicyVersion?`). `totalScore` (0–100) = **Σ pct×weight / Σweight** (CÓ trọng số) — là **snapshot phục vụ ranking B2B** (E4). **B2C** (`campaignId=null`): điểm hiển thị cho user = **trung bình cộng** tính riêng ở **BC9** (`result.overallScore`), **KHÔNG** đọc `totalScore` của event này (tránh lệch INT-10).
  - **RNK1 · HĐ-1/HĐ-2:** `scoringInputs` (`campaign_rankings.scoring_inputs`) thêm `seedAnswered`/`seedTotal` (câu GỐC `kind='Seed'` **có ghi âm và `reject_reason` ≠ `'no_speech'`** / tổng — CAMP-21 2026-09-11: bài VAD xác nhận im lặng KHÔNG tính; `answered` dùng CÙNG vị ngữ; `reject_reason IS NULL` vẫn tính ⇒ không hồi tố) + `skipPenalty` + `criteria[].criterionId` (B1 để null; B4 điền). Khi buổi `skip_penalty=true`: `totalScore = clamp(expr × seed_completeness, 0, 100)` — `SessionScoringNotifier` áp qua `Isas.Shared.Scoring.SkipPenaltyRule.Apply`, **cùng hàm** `ScoringPolicyService` (preview/áp) dùng ⇒ điểm chấm thường = điểm preview = điểm áp. Snapshot ghi TRƯỚC RNK1 (thiếu `seedTotal` ⇒ null) ⇒ không phạt, hàng lịch sử giữ nguyên điểm.
...
| PUT | `/api/admin/prompts/{key}` | Admin | Tạo **version mới**; 400 khi khoá lạ / body rỗng / >8.000 ký tự / chứa delimiter khung. ✅ **2026-09-16 (B4):** snapshot **`updatedByEmail`** từ claim `email` của JWT (mẫu E11c `ranking_overrides.actor_email`; GEN-3 không tra Auth) — response `GET`/`history`/`PUT` đều mang; `null` = bản cũ hoặc token thiếu claim ⇒ UI hiện "không rõ người sửa", KHÔNG hiện Guid. Cột `prompt_templates.updated_by_email varchar(256)` (migration `AddPromptTemplateUpdatedByEmail`, additive) |
| DELETE | `/api/admin/prompts/{key}` | Admin | Về bản mặc định = deactivate; **giữ lịch sử** |

> 🔒 **B10 (2026-09-16) — mọi DTO body của endpoint admin-only (`Roles="Admin"` là role DUY NHẤT) mang `[JsonUnmappedMemberHandling(Disallow)]`:** khoá JSON lạ ⇒ `JsonException` ⇒ **400**, thay vì bị System.Text.Json nuốt im lặng thành 200 no-op (cách màn rubric admin hỏng gần 1 tháng: FE gửi `description`, BE đợi `descriptor`). Áp cho `UpdatePromptTemplateRequest` · `UpsertAdminRubricRequest` (+ kiểu lồng `AdminRubricCriterionInput`/`AdminRubricLevelInput`) · `AdminRubricPreviewRequest` · `UpdateRoadmapLevelThresholdsRequest` · `CreateKnowledgeRequest` · `Context7IngestRequest`. **KHÔNG áp** cho DTO callback/internal/public (hợp đồng nới theo từng bước deploy phải tha thứ). Guard reflection `AdminRequestDtoDisallowTests` quét mọi action admin ⇒ DTO admin thêm sau tự bị bắt. `DeliveryMetricsDto` lồng trong `AdminRubricPreviewRequest` cố ý không áp (kiểu dùng chung với đường callback).
