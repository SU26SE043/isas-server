...
- **Warm cache ngay khi sinh câu:** `/generate-questions` làm nóng các câu seed vừa sinh, `/decide-next` làm nóng câu adaptive vừa quyết định. Warmup chạy nền, best-effort và bật bằng `TTS_PREWARM_ENABLED` (mặc định `true`) nên không kéo dài response sinh câu. Batch dùng **2 lane** (`TTS_PREWARM_CONCURRENCY`) để câu 4/5 không phải chờ toàn bộ câu trước, nhưng vẫn chặn burst cả batch vào quota model preview.
