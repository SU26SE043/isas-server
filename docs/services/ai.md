...
> ✅ **14/14 endpoint đều gate `X-Internal-Token`** (Q2, 2026-08-07; `/face-detect` thêm 2026-09-17); chỉ `GET /health` để trần. Các dòng `— (nội bộ)` ở cột gateway là những endpoint **chưa bao giờ** có route gateway; số còn lại đã bị gỡ khỏi gateway public từ GEN-7.
...
- `generate-lesson-theory`: req `{ jobCategory, level, lessonTitle, focusCriteria: string[], weaknesses?: string[], mode? }` → res `{ theoryMarkdown, resources[] }` (tiếng Việt, có ví dụ — nội dung ôn tập lý thuyết cho lesson). Interview lưu `roadmap_lessons.theory_content` + `roadmap_lessons.resources` (**lazy** — sinh 1 lần khi mở lesson đầu tiên; nay còn **prewarm nền** + **single-flight** phía Interview — xem [interview.md](interview.md) §lessons).
  - ⏱ **Ngân sách (2026-09-15):** `LESSON_THEORY_THINKING_BUDGET=1024` (`-1` quay lui) + `LESSON_THEORY_MAX_OUTPUT_TOKENS=12288` (`0` = không cắt). Đo prod 2026-09-14: 38–52s/lượt, **~nửa output token là suy luận ẩn**; A/B 3 bài thật × 5 mức (`scripts/benchmark-lesson-theory.py`): mặc định p50 40,5s (thoughts tới 7.975) → 1024 p50 **22s** (thoughts ~800), độ dài bài/số mục/phủ lỗi **không đổi** — phần còn lại là chữ (~3–5k token ở ~200 token/s). ⚠ Trần output tồn tại vì đã bắt được **hai** lượt chạy loạn (64.768 token/254s trong A/B; 15.569 token/65s trên dev) — với Gemini 2.5 **suy luận tính vào `max_output_tokens`** (đo: 15.569 + 799 = 16.368 ≈ trần cũ 16.384). Lượt bị cắt → không phải JSON → vòng viết lại (`lesson_theory_max_attempts=2`). Mỗi lượt Gemini log một dòng `[⏱] lesson-theory attempt=i/n elapsed= finish_reason= candidates_token_count= thoughts_token_count= defects=` (kể cả lượt bị trả lại); lượt không phải JSON log WARNING kèm 200 ký tự đầu + 300 ký tự đuôi.
  - `generate-roadmap` cùng lỗ: `ROADMAP_THINKING_BUDGET=1024` — đường này do suy luận chi phối (chỉ ~200 token JSON): mặc định 7–11s → ~6s, cùng số chặng/bài (n=4, chỉ kết luận về hướng).
  ...
  - 🔴 **URL do LLM sinh = ảo giác kinh điển.** Hai lớp phòng thủ, cố ý có CẢ HAI vì lớp 1 không đáng tin một mình: **(1) prompt** cấm đoán/ghép url, không chắc thì để trống; **(2) allowlist TÊN MIỀN** (`app/resources.py::sanitize_resources`) — giữ url chỉ khi **https** + host khớp **đầy đủ** (không substring, chặn `mozilla.org.evil.com`) một nguồn chính chủ đã biết; host lạ → **bỏ url, GIỮ tên tài liệu**. Cũng chặn `javascript:`/`data:`/`file:`/scheme-relative.
    - ✅ **2026-09-15 — câu "bỏ url, GIỮ tên" nay mới ĐÚNG với code.** Trước đó `sanitize_resources` bỏ **cả mục** khi url trống/host lạ — tự mâu thuẫn với chính prompt ("không chắc thì ĐỂ TRỐNG url") ⇒ bài BA 0 tài nguyên trên prod. Kèm: prompt đưa ví dụ host **theo NGÀNH** (`resource_host_examples(job_category)`) thay vì 12 host đầu theo ABC (toàn dev-tool — bài BA chưa từng thấy host ngành mình); allowlist thêm host BA khai **cả `www.` lẫn trần** (khớp chính xác). Nguyên tắc: host admin đã curate vào kho RAG đủ tin để làm link.
...
**`POST /api/v1/embed`** (gate `X-Internal-Token`, fail-closed): `{texts: string[], taskType: "RETRIEVAL_DOCUMENT"|"RETRIEVAL_QUERY"}` → `{vectors: number[][], dim: 768, model: "gemini-embedding-001"}`. Model `gemini-embedding-001` **đa ngôn ngữ** (query tiếng Việt tìm thẳng chunk tiếng Anh, không cần dịch), `output_dimensionality=768` (Matryoshka), `task_type` tách document/query. Lỗi provider → 502 **kèm log WARNING + `detail` nguyên văn** (Interview giữ lại vào `AiServiceException`). Config `embed_model`/`embed_dim`.
- ✅ **BK34 (2026-09-15): provider chia lô ≤100 text/lời gọi** — Gemini `batchEmbedContents` nhận **tối đa 100 request/lô** (đo thật: 90 OK · 101 → `400 "at most 100 requests can be in one batch"`). Trước đó cả nguồn đi một lời gọi ⇒ mọi trang > 100 chunk (Atlassian, Agile Alliance, Camunda) nạp thất bại và lỗi bị nuốt hai lần (AIService không log, .NET vứt `detail`); chẩn đoán cũ "chunk quá cỡ" SAI. Giới hạn kích thước từng text vẫn là 2.048 token của model (Chunker cap 1.800 ký tự).
...
**Bật bằng DỮ LIỆU, không bằng cờ:** request mang `criteria: [{criterionId, name, description?}]` (tiêu chí **NỘI DUNG**; SC2 · T5 2026-09-13: `description?` additive, in vào khối gắn nhãn khi có — Campaign gửi mô tả HR gõ để AI nhắm đúng ý) ⇒ gắn nhãn. **Vắng/rỗng ⇒ prompt giữ NGUYÊN XI, không thêm một chữ nào** — đúng mẫu `criteria` của C14 ở `build_cv_analysis_prompt`, và đây là thứ giữ cho Campaign B2B cùng mọi caller cũ khỏi phải sửa gì. Có test khoá bất biến đó (`build_prompt` có và không có tham số phải bằng nhau **từng ký tự**).
...
## Phát hiện ≥2 giọng nói (`multi_voice`) 🧪 **AC1/B5 — THỬ NGHIỆM, mặc định TẮT (2026-08-28)**
Chạy **trong worker chấm**, SAU khi đã có transcript (`app/multi_voice.py`). 🔴 **KHÔNG chạm `/decide-next`** — đường đó ĐỒNG BỘ trong request upload, đã đo **96s** trên bài dài (trần decider 90s).
- **Ba cổng chạy** (thiếu một là bỏ qua êm): `MULTI_VOICE_ENABLED` · job có **`CampaignId`** (B2B — van **BC-6**, B2C là luyện tập nên không giám sát) · **`AttemptNo == 1`** (E10 chấm cùng answer N lần ⇒ chạy mọi attempt là nhân bản cả chi phí lẫn cờ). Cổng nằm trong `resolve_context()` — hàm RIÊNG để unit-test được (bài học C14: guard trong `main()` thì gỡ đi không test nào đỏ).
- **Pipeline:** audio → VAD (**dùng lại `transcriber.VAD_OPTIONS`** của F11, không khai bộ mới) → nối thành trục tiếng-nói → cửa sổ **1,5s / bước 0,75s** (trần **80** cửa sổ; vượt thì **lấy mẫu ĐỀU**, KHÔNG cắt đuôi) → fbank Kaldi (`app/fbank.py`, numpy thuần) → CMN → nhúng ONNX **WeSpeaker CAM++ VoxCeleb** (28 MB, tải lúc **build image**, ghim URL + sha256) → gom cụm **average-linkage** cosine, cắt 2 cụm.
- **Gắn cờ ⇔ CẢ HAI đạt:** cụm nhỏ hơn ≥ **3,0s** tiếng nói **VÀ** khoảng cách cosine giữa hai tâm ≥ **0,50**. Thời lượng quy theo **bước nhảy** (không theo bề rộng cửa sổ — chồng 50% nên tính bề rộng là đếm đôi, thổi phồng cáo buộc).
- **Callback:** `POST {CAMPAIGN_CALLBACK_BASE}/internal/session-flags` + `X-Internal-Token` (GEN-1, không qua gateway), `signalType="multi_voice"`. **Note TẤT ĐỊNH** `answer {id}: ~{N}s giọng thứ hai` — đó là **khoá dedup** của CampaignService `(SessionId, SignalType, Note)`; đường chấm cố ý chạy lại (self-consistency + `StuckAnswerRepublisher`) nên note đổi mỗi lượt là HR thấy `multi_voice: 3` cho MỘT lần nghi vấn.
- **An toàn:** mọi exception (kể cả callback hỏng) đều bị **nuốt + log** — lượt chấm là **đường tiền** (PAY-13), cờ chỉ là phụ phẩm. Detector chạy **sau `message.ack()`**.
- **Không phụ thuộc mới:** chỉ `onnxruntime` + `numpy` (đã có sẵn cho insightface). **KHÔNG** torch/pyannote/speechbrain — fbank Kaldi tự cài bằng numpy, khoá vào bản tham chiếu `kaldi-native-fbank` bằng `tests/test_fbank_kaldi_parity.py` (lệch tối đa **3,3e-4**). Image ~594 MB → ~622 MB. Chi phí: **~1,5s CPU/answer** (đo trên bản ghi thật 28s).
- 🔴 **CHƯA ĐỦ BẰNG CHỨNG ĐỂ BẬT.** Hiệu chuẩn 2026-08-28: **0/25 dương-tính-giả** (7 ghi âm thật + 18 ca ghép cùng người) và bắt đúng **2/2** ca dương cần bắt, khe an toàn `0,407 … 0,527`. NHƯNG toàn bộ ghi âm thật hiện có **chỉ của MỘT người** (13 bản, F0 116–158 Hz — kiểm bằng F0 độc lập với model) và **mọi ca dương đều là giọng tổng hợp `say`** ⇒ **chưa ai đo tính năng này trên hai người thật**. Điều kiện bật: thu ≥10 buổi có hai người thật + ≥20 buổi một người, hiệu chuẩn lại.
- ⚠ **v1 KHÔNG hứa:** giọng CHỒNG nhau (đo được: không bắt), giọng thì thầm/xa mic, phân biệt >2 người (mọi thứ ngoài giọng chính bị gom làm "giọng thứ hai").
- ⚠ Cờ này chỉ mở cổng phía AIService. Cờ tới được HR còn cần **`campaign.anti_cheat_enabled`**: `multi_voice` KHÔNG thuộc `IdentitySignals` nên `face_verify_enabled` một mình **không đủ** để `SessionFlagController` lưu.
...
- **✅ Sinh mốc + chấm thử (CAMP-16/19, 2026-08-13):** `POST /suggest-criterion-levels` (AI đề xuất mốc, **không fallback bịa dải mặc định** — lỗi → ném → .NET trả 502, vì HR sẽ tin `"Mức 3/10"` là do AI viết) · `POST /score-preview` (viết 3 bài mẫu rồi **gọi thẳng `score()` từng bài một**, `temperature=0.0` như production, `delivery=None` cho bài AI). **2026-09-16:** thêm `includeAiSamples` (mặc định `true`; `false` ⇒ bỏ sinh 3 bài, chỉ chấm `customAnswer` — thiếu ⇒ 400) và `customDelivery` (số đo F11 của CHÍNH bản ghi người dùng — chỉ gắn vào bài `Custom`, không gắn vào bài AI); `/transcribe` trả thêm `rejectReason` (`no_speech` khi cổng VAD không thấy tiếng nói). Prompt viết bài mẫu hiệu chuẩn cùng ngày (Yếu = hiểu SAI, trần/sàn từng mốc, `plan` bắt buộc trước `text`, cấm ký hiệu code) — xem `test_score_preview.py`. Hai hàm `build_scoring_prompt`/`score` **không được sửa** — có **golden test khoá theo hash**; đỏ nghĩa là prompt chấm vừa đổi, **cập nhật hash KHÔNG phải cách sửa**. Ba field `expected*` bị **lột trước khi vào prompt chấm** (có test khoá) — để lọt là mách đáp án cho chính bộ chấm, khiến mọi con số expected-vs-actual sau đó vô nghĩa mà lại trông rất thuyết phục.
