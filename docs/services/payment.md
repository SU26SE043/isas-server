...
- Ánh xạ đặt ở `RefundPayout:BankBinMap` (config, sửa không cần deploy), dạng env `RefundPayout__BankBinMap__<citad>=<bin>` — ví dụ `RefundPayout__BankBinMap__01203001=970436`. **Cố ý không hardcode bảng này trong code**: đó là dữ liệu tra cứu của bên thứ ba, đổi khi có ngân hàng mới, và mỗi dòng phải được người xác nhận trước khi tiền đi theo nó. Đối chiếu BIN tại [api.vietqr.io/v2/banks](https://api.vietqr.io/v2/banks).
