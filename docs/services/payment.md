...
**`GET /payment/package`** · **`GET /payment/package/{id}`** — Gói prepaid đang bán. Public. → `ProductPackage[]` / `ProductPackage`. Cả hai **lọc `is_active=true`**; GET-by-id không thấy (id lạ **hoặc** gói đã ngừng bán) → **404** `{message:"Package not found"}`. ✅ **2026-09-17 (đợt D admin):** `?includeInactive=true` trả cả gói đã ẩn — **chỉ có hiệu lực khi caller có role Admin** (`User.IsInRole`); người lạ/candidate truyền cờ vẫn chỉ thấy gói đang bán (giá chưa công bố không lộ). Vì sao: DELETE là soft-delete và cả 2 GET đều lọc ⇒ gói đã ẩn biến mất khỏi mọi list, không "Bán lại" được từ UI (PUT `isActive:true` cần id). *(Fix e2e 2026-07-18: trước đây GET-by-id id lạ ném NRE → **500** trên endpoint `[AllowAnonymous]`, và gói đã soft-delete vẫn trả 200 — lệch GET catalog.)*
...
**`/payment/admin/plans`** — CRUD catalog tier, chỉ `Admin`. `DELETE /{id}` soft-deactivate (`is_active=false`), không xoá row lịch sử đã được package/subscription tham chiếu; gói mặc định (`free` B2C · `starter` B2B) → **400** `{message}` (✅ 2026-09-17: trước đó `ArgumentException` không được catch ⇒ **500**). `PlanResponse` nay trả thêm **`entitlementsJson`** (✅ 2026-09-17, additive) để client echo khi PUT — `PlanRequest.ApplyTo` luôn ghi đè `EntitlementsJson`/`EntitlementsVersion`, không echo là mỗi lần sửa reset JSON về `"[]"` im lặng. Validation: Metered cần quota dương; B2C không có B2B cap; Unlimited chỉ khi `Tiering:AllowUnlimitedPlans=true`. Sửa catalog không hồi tố subscription snapshot; chỉ activation/mua mới dùng catalog mới.
...
- ✅ **2026-09-15 — mọi đường lật trạng thái đều là `ExecuteUpdate … WHERE status = Pending`** (webhook · poll · sweeper · **huỷ đơn**). `CancelOrderAsync` cũ là `FindAsync` + `SaveChanges` không guard ⇒ webhook Paid lật đơn giữa lúc gọi PayOS huỷ là bị ghi đè thành `Cancelled`; nay huỷ đi qua `IPayOsCancelClient` (mock được), PayOS từ chối → hỏi lại DB: đơn đã rời Pending ⇒ **400** nêu đúng trạng thái, còn Pending thật ⇒ **502** (`PaymentGatewayException`), không còn 500 stack thô.
- ✅ **Poll thấy PayOS `Cancelled` → đóng `Cancelled` NGAY** (link đã huỷ, PayOS không nhận tiền nữa) — trước đây giữ Pending tới sweeper (~45') và FE quay 90s. **CHỈ `Cancelled`**: `Expired`/`Failed`/`Processing`/`Underpaid` phía PayOS vẫn để `OrderExpiryReconciler` xử sau ân hạn (đóng mù là mất tiền thật).
...
- **✅ R1 (2026-07-20) — lưới cuối cho chỗ giữ của session ĐÃ TERMINAL:** DB18 chỉ dọn ca session **không tồn tại**; ca *session tồn tại nhưng đã terminal mà reservation vẫn `Reserved`* trước R1 **không ai dọn** → rò credit hai chiều (đo được trên production 2026-07-20: 1 ca `SessionAbandoned` = user mất oan 1 credit · 2 ca `Scored` = org/user được buổi miễn phí). Nguồn gốc là mất event settle (binding RabbitMQ vắng một cửa sổ, message bị topic exchange vứt im lặng trong khi outbox đã đóng dấu `published_at`) — lỗ cần vá là **thiếu lưới cuối**. Interview `/internal/sessions/exists` nay trả thêm `states[{sessionId,status}]` (**ADDITIVE**, status = tên enum dạng string, GEN-2); reconciler phân nhánh:
  ...
  **🔴 AN TOÀN — nhánh CONSUME chặt hơn nhánh RELEASE** (trước R1 sai thì hoàn nhầm, còn cứu được; nay sai là **mất tiền người dùng**, không tự phục hồi): ①chỉ consume khi khẳng định **DƯƠNG** `Scored` — whitelist một-phần-tử, **KHÔNG có nhánh `default → Consume`**; ②công tắc riêng `OrphanReconcile:ConsumeTerminalScored` (mặc định `true`) tắt được nhánh trừ tiền bằng env mà **vẫn giữ nhánh release chạy**, khỏi rollback image; ③mốc `OrphanReconcile:ConsumeFromUtc` — **không cấu hình ⇒ lấy mốc KHỞI ĐỘNG reconciler** ⇒ "chỉ từ nay về sau"; chỗ giữ `Scored` **cũ hơn mốc → SKIP + log Warning, KHÔNG release** (release buổi đã chấm = tặng buổi miễn phí, đúng bug đang sửa) → tồn đọng để **người** đối soát tay (OPS2), vì đó là hệ quả sự cố hạ tầng của ta chứ không phải lỗi người dùng.
  ...
  ⚠ **Hệ quả phải nhìn thấy trong log:** restart dịch vụ đẩy mốc mặc định tiến lên ⇒ chỗ giữ sinh ngay trước restart có thể không bao giờ được consume (reconciler log mốc + nguồn mốc lúc khởi động, và log từng ca bị bỏ). ✅ **2026-09-15 — đo prod: mốc = đúng giờ container lên (13:37:52Z), tức TRÔI theo mỗi lần CI deploy.** Thêm **`OrphanReconcile:ScoredConsumeFromUtc`** — mốc RIÊNG cho nhánh Scored (thứ tự: `ScoredConsumeFromUtc` → `ConsumeFromUtc` → khởi động); compose đặt mặc định `2026-07-24T00:00:00Z` (ngày R1 lên prod), chỗ giữ cũ hơn vẫn SKIP. 🛑 **KHÔNG dùng `ConsumeFromUtc` để ghim mốc Scored** — nó là **cutover PONR1**: đặt là `CreditEventHandler` ngừng release session bỏ ngang và reconciler CONSUME chúng, trong khi Interview vẫn `Billing:ConsumeAtQuestionGeneration=false` ⇒ người bỏ ngang bị trừ tiền. Có test khoá mốc riêng **không lây** sang gate PONR1.
...
- ✅ **2026-09-15 — webhook ĐỐI CHIẾU SỐ TIỀN:** `data.amount` là tiền của **MỘT giao dịch** chuyển vào, không phải của link (PayOS cho trả nhiều lần: `amountPaid`/`amountRemaining`, trạng thái `UNDERPAID`). `WebhookController` truyền `data.Amount` vào `ApplyPaidWebhookAsync(orderCode, amountPaidVnd, …)`; **thiếu tiền** → hỏi lại PayOS: link đã `Paid` (chuyển 2 lần gộp đủ) ⇒ áp; chưa `Paid` / không hỏi được ⇒ **GIỮ Pending**, ghi bằng chứng `payment_transactions.status='underpaid'`, trả `Underpaid` (fail-closed — poll P3 và sweeper vẫn cứu được về sau vì cả hai hỏi PayOS trước khi áp). Overload cũ không số tiền (poll/sweeper) đi thẳng. *Trước bản này `success=true` là lật Paid + cộng ĐỦ credit bất kể chuyển bao nhiêu.*
- **Active polling:** FE gọi `GET /order/{id}/status`; nếu server **chưa** nhận webhook → server **chủ động gọi PayOS get-payment-info đối soát ngay** (cứu ca webhook delay/drop làm FE đứng hình). ✅ **Bằng chứng poll chỉ ghi khi trạng thái PayOS ĐỔI** so với dòng cuối của đơn (trước: mỗi lượt poll 2s là một dòng `pending` y hệt — đo prod 404/427 dòng `payment_transactions` là rác poll, một đơn 122 dòng). FE B2C nhận `cancel=true`/`status=CANCELLED` từ PayOS thì chỉ hỏi **một lần** rồi sang trang thất bại, không poll 45 lượt.
