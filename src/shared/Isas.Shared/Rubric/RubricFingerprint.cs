...
/// <summary>
/// Dạng chuẩn tắc (canonical) + vân tay của MỘT BỘ THƯỚC ĐO.
///
/// <para>Một nguồn duy nhất cho ba việc, cố ý KHÔNG tách thành ba cơ chế so sánh:</para>
/// <list type="number">
/// <item>Trả lời <i>"có thật sự đổi thước đo không"</i> khi quyết định bump phiên bản —
/// <c>campaigns.rubric_version</c> (CAMP-18) và bộ chuẩn B2C do admin quản.</item>
...
/// <item>Đối chiếu bộ đã materialize sang Interview với bộ gốc ở Campaign.</item>
/// </list>
/// </summary>
public static class RubricFingerprint
{
    ...
    /// "chỉ sửa mốc" với "sửa chính bộ tiêu chí" — hai việc chịu hai luật khác nhau (CAMP-2 vs CAMP-18).
    /// </param>
    public static string Canonicalize(IEnumerable<RubricCriterionSnapshot> criteria, bool includeLevels = true)
    ...
    public static List<CanonicalCriterion> ToCanonical(
        IEnumerable<RubricCriterionSnapshot> criteria, bool includeLevels = true)
        => criteria
            // OrderNo là khoá sắp chính; Name là chốt chặn cho bộ đang dựng trong bộ nhớ mà OrderNo
            // chưa gán xong. Ordinal: so byte, không theo locale.
            .OrderBy(c => c.OrderNo)
    ...
    public static string Compute(IEnumerable<RubricCriterionSnapshot> criteria, bool includeLevels = true)
