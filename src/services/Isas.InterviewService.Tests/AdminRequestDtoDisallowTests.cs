...
{
    private static readonly Assembly ServiceAssembly = typeof(AdminPromptController).Assembly;
    ...
    public void KhoaLa_NemJsonException_KhoaDung_VanParse()
    {
        ...
        Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<UpdatePromptTemplateRequest>("{\"body\":\"x\",\"bodyy\":\"y\"}", opts));
        Assert.NotNull(JsonSerializer.Deserialize<UpdatePromptTemplateRequest>("{\"body\":\"x\",\"changeNote\":null}", opts));
    }

    /// <summary>
    /// Disallow KHÔNG lan xuống kiểu lồng: <c>UpsertAdminRubricRequest</c> đúng mà phần tử
    /// <c>criteria[].levels[]</c> vẫn nuốt khoá lạ thì FE gửi <c>description</c> thay <c>descriptor</c>
    /// ở mốc vẫn 200 no-op — đúng ca đã hỏng thật. Guard reflection ở trên chỉ thấy kiểu top-level,
    /// nên khoá riêng hai kiểu con ở đây.
    /// </summary>
    [Theory]
    [InlineData(typeof(AdminRubricCriterionInput))]
    [InlineData(typeof(AdminRubricLevelInput))]
    public void KieuLongCuaRubricUpsert_CungPhaiDisallow(Type nested)
    {
        var attr = nested.GetCustomAttribute<JsonUnmappedMemberHandlingAttribute>(inherit: false);
        Assert.True(attr is not null && attr.UnmappedMemberHandling == JsonUnmappedMemberHandling.Disallow, nested.Name);
        var opts = new JsonSerializerOptions(JsonSerializerDefaults.Web);
        Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<UpsertAdminRubricRequest>(
            """{"criteria":[{"id":"00000000-0000-0000-0000-000000000001","description":null,"levels":[{"score":0,"description":"sai tên khoá"}]}]}""", opts));
    }
}
