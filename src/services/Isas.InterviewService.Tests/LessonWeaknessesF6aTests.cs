...
public class LessonWeaknessesF6aTests
{
        ...
        TestDb t, Roadmap roadmap, Guid candidateId, decimal? threshold = null)
    {
        ...
        gen.Setup(g => g.GenerateLessonTheoryAsync(
                ...
                It.IsAny<IReadOnlyList<GroundingChunk>?>(), It.IsAny<IReadOnlyList<CriterionEvidence>?>(), It.IsAny<RoadmapMode>(), It.IsAny<CancellationToken>(), It.IsAny<IReadOnlyList<RoadmapMistake>?>()))
            .Callback<string, string, string, IReadOnlyList<string>, IReadOnlyList<string>?, IReadOnlyList<GroundingChunk>?, IReadOnlyList<CriterionEvidence>?, RoadmapMode, CancellationToken, IReadOnlyList<RoadmapMistake>?>(
                (_, _, _, _, weaknesses, _, _, _, _, _) => captured = weaknesses)
