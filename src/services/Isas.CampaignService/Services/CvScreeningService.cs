    ...
    public class CvScreeningService : ICvScreeningService
    {
        ...
        public async Task<int> PublishScreeningJobsAsync(Guid employerId, Guid campaignId, CancellationToken ct)
        {
            ...
            var candidates = await _db.CampaignCandidates
                .Where(c => c.CampaignId == campaignId && c.Status == CandidateStatus.Filtered)
                .ToListAsync(ct);
            ...
            foreach (var cand in candidates)
            {
                try
                {
                    ...
                    cand.Status = CandidateStatus.Analyzing;
        ...
        public async Task<CvResultOutcome> SaveCvResultAsync(Guid candidateId, CvResultCallbackRequest req, CancellationToken ct)
        {
            ...
            candidate.Status = CandidateStatus.Analyzed;   // recover cả từ Analyzing lẫn AnalysisFailed (doc)
        ...
        public async Task<CvFailedOutcome> MarkCvFailedAsync(Guid candidateId, string? reason, CancellationToken ct)
        {
            ...
            candidate.Status = CandidateStatus.AnalysisFailed;
            ...
            Guid employerId, Guid campaignId, string? status, int? minScore, string? skill, string? sort, CancellationToken ct)
        {
            ...
            var q = _db.CampaignCandidates.Where(c => c.CampaignId == campaignId);

            if (!string.IsNullOrWhiteSpace(status) &&
                Enum.TryParse<CandidateStatus>(status, ignoreCase: true, out var st))
            ...
            Guid employerId, Guid campaignId, Guid candidateId, PatchCandidateRequest req, CancellationToken ct)
        {
            ...
            if (req.Email is not null)
            {
                ...
                var dup = await _db.CampaignCandidates.AnyAsync(
