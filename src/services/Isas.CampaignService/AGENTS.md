...
| POST | `/campaign/{id}/start-now` | **CMP3-B4** kéo `start_at` về hiện tại (không body). Không Active / có ca thi → **409**. **CMP4-B3 — `expires_at` đã qua → **409** (start-now KHÔNG tự dời hạn). `start_at` quá khứ → no-op. Hợp lệ → 1 audit `StartEarly` (mốc cũ) + outbox `OpenedEarly` cho lời mời **chưa revoke VÀ chưa hết hạn** |
