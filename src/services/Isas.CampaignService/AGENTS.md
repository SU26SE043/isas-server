...
| POST | `/campaign/{id}/start-now` | **CMP3-B4** kéo `start_at` về hiện tại (không body). Không Active / có ca thi → **409**. **CMP4-B3 — `expires_at` đã qua → **409** (start-now KHÔNG tự dời hạn). `start_at` quá khứ → no-op. **D32: wizard FE không gọi lúc triển khai (bỏ ô "Mở ngay khi triển khai" 21/09), chỉ trang chi tiết dùng.** Hợp lệ → 1 audit `StartEarly` (mốc cũ) + outbox `OpenedEarly` cho lời mời **chưa revoke · chưa hết hạn · `email_sent_at != null`** (CMP4-B5). Nhánh `OpenedEarly` của consumer: dedup theo messageId (in-process TTL 6h, route A) + trần thất bại `OpenedEarlyEmail:MaxAttempts` (5) → thôi requeue; nhánh thư mời KHÔNG đụng |
...
> **SC2 · W2 (2026-09-13):** `CampaignResponse.questionBank` += `coverageWarnings: [{criterionId,name}]` (tiêu chí `WhenTargeted` không câu nào nhắm — CHỈ cảnh báo, không chặn publish, luôn có mặt, cả trên danh sách) và `warnings` += `K_BELOW_CRITERIA_GROUPS: …` khi `K < số tiêu chí chính distinct` (`targetCriterionIds[0]`; K null / 0 nhãn ⇒ không bắn) ⇒ publish **400 `QUESTION_BANK_INVALID`** (D-5). `QuestionPoolSelector` chia rổ theo tiêu chí CHÍNH, câu chưa gắn nhãn theo `questionGroup` như trước.
...
- **E11c** — `GET …/results/{sessionId}/override-history` (lịch sử điều chỉnh của HR, mới-nhất-trước, từ bảng append-only `ranking_overrides`; `actorEmail` null = không biết) · `GET …/results/{sessionId}/answers/{answerId}/audio` (proxy Interview internal, key S3 không lộ; 404 khi không có audio, 502 khi Interview lỗi) · `…/transcript` thêm `answerId/kind/answerStatus/rejectReason/durationSec/hasAudio/sampleAnswer/deliveryMetrics/levelMatched`. Chi tiết: `docs/services/campaign.md` §E11c + §`ranking_overrides`.
...
| PUT | `/campaign/{id}/job-needs` | ✅ HR chốt **nhu cầu công việc** dùng để sàng CV (replace-all). Body `[{needId?,category,text,isMustHave?}]`, `category ∈ Technical·WorkStyle·Communication·Growth`. `needId` echo lại để kết quả sàng đã lưu còn trỏ đúng dòng (mẫu F10); **`source` KHÔNG nhận từ client** — server sở hữu. **CMP4-B1 — cửa sửa đo CÙNG THƯỚC với luật-lọc-cứng: campaign đã có BẤT KỲ `cv_submission` nào (Filtered/Analyzing/…) HOẶC `Closed`/`Archived` → **409** (đổi thước sau khi có người bị đo là không so sánh được — `job_needs` không mang nhãn phiên bản). Gửi `[]` khi đã có `cv_submission` cũng **409**; wipe khi CHƯA có ai → 200. `category` lạ → **400** |
...
| domain | varchar(100)? | ⚠ **DƯ — target: bỏ.** Free-text. ⚠ NHƯNG nhánh sàng CV B2B đang đọc (`BuildCriteriaAsync`/`BuildJobNeedsAsync` `jobCategory`, `CvScreeningJob.domain`, `POST /questions/generate`) → **CMP4-B2** khoá `domain` (như `language`) bằng cùng thước với 3 luật lọc cứng: có `cv_submission` / `Closed`/`Archived` → `PUT /campaign` **409** |
...
| job_needs | jsonb? | `[{needId,category,text,source,isMustHave}]` — **nhu cầu công việc dùng để SÀNG CV** (khác `campaign_criteria` = thước chấm buổi phỏng vấn). AI suy từ `jd_text` lúc publish → HR sửa qua `PUT /job-needs` **khi campaign CHƯA có `cv_submission` nào** (CMP4-B1 — cùng thước luật-lọc-cứng; đã có ứng viên / `Closed`/`Archived` → 409). Chốt MỘT LẦN cho cả campaign để mọi ứng viên đo cùng thước (mẫu CAMP-10). `source` = `AiSuggested`/`HrEdited`, **server sở hữu** (F10). null/rỗng ⇒ **chưa sàng CV được** |
...
> Tạo/reissue invitation → ghi outbox-row **CÙNG transaction** (thay publish best-effort SAU commit) → `OutboxDispatcher` (BackgroundService, `Outbox:*`) publish `campaign_invitation_email_queue` at-least-once (broker down → giữ null → gửi lại). Consumer dedup **nhánh thư mời** theo `campaign_invitations.email_sent_at` (bền vững). **CMP4-B5 — nhánh `OpenedEarly`** dedup + trần thất bại riêng, keyed by messageId của outbox row (in-process TTL 6h, route A); ≥ N lỗi (`OpenedEarlyEmail:MaxAttempts`=5) → thôi requeue + log Error. Nhánh thư mời KHÔNG đụng. Mirror DB2 (Interview). *(cv_screening dùng reconciler-theo-state riêng — `last_screening_published_at` + `StuckScreeningRepublisher`, không qua bảng này.)*
