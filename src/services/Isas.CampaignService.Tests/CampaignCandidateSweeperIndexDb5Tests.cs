...
// DB5: StuckScreeningRepublisher (C15) quét cv_submission (DB16 ex campaign_candidates) mỗi 2' theo
// predicate (Status, LastScreeningPublishedAt) — KHÔNG có campaign_id → index (campaign_id, status) vô dụng.
...
public class CampaignCandidateSweeperIndexDb5Tests
{
    ...
    public void Model_co_index_status_lsp_dung_thu_tu()
    {
        ...
        var entity = tdb.Db.Model.FindEntityType(typeof(CvSubmission));
        Assert.NotNull(entity);

        var expected = new[]
        {
            nameof(CvSubmission.Status),
            nameof(CvSubmission.LastScreeningPublishedAt)
        };
        ...
        Assert.False(index!.IsUnique);                          // non-unique (nhiều CV cùng status)
        Assert.Equal("ix_cv_submission_status_lsp",
            index.GetDatabaseName());                           // tên rút gọn như migration (DB16)
    }
}
