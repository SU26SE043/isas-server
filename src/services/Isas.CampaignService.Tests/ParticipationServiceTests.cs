...
public class ParticipationServiceTests
{
    ...
    public async Task Join_Duong1_TaoMembershipJoined_TraTokenVaJoined()
    {
        ...
        var svc = NewService(tdb.NewContext());
        var res = await svc.JoinCampaignAsync(RawTokenOf(inv), inv.Email, default);
    ...
    // BK26 — caller có JWT Candidate nhưng email khác token mời: chặn TRƯỚC provision và mutation.
    [Fact]
    public async Task Join_EmailCallerKhacInvitation_403_KhongProvisionKhongTaoMembership()
    {
        using var tdb = new CampaignTestDb();
        var camp = CampaignTestDb.NewCampaign(Guid.NewGuid(), CampaignStatus.Active);
        var inv = NewInvitation(camp.Id, "duoc-moi@acme.test");
        tdb.Db.Campaigns.Add(camp);
        tdb.Db.CampaignInvitations.Add(inv);
        await tdb.Db.SaveChangesAsync();

        var auth = DefaultAuth();
        var svc = NewService(tdb.NewContext(), auth);

        await Assert.ThrowsAsync<UnauthorizedAccessException>(() =>
            svc.JoinCampaignAsync(RawTokenOf(inv), "nguoi-khac@acme.test", default));

        auth.Verify(x => x.ProvisionCandidateAsync(
    ...
    [InlineData("")]
    [InlineData("   ")]
    public async Task Join_EmailCallerThieu_403_KhongTaoMembership(string? callerEmail)
    {
        using var tdb = new CampaignTestDb();
        var camp = CampaignTestDb.NewCampaign(Guid.NewGuid(), CampaignStatus.Active);
        var inv = NewInvitation(camp.Id);
        ...
        await Assert.ThrowsAsync<UnauthorizedAccessException>(() =>
            NewService(tdb.NewContext()).JoinCampaignAsync(RawTokenOf(inv), callerEmail, default));
    ...
    [Fact]
    public async Task Join_EmailCallerKhopKhongPhanBietHoaThuongVaKhoangTrang_ThanhCong()
    {
        using var tdb = new CampaignTestDb();
        var camp = CampaignTestDb.NewCampaign(Guid.NewGuid(), CampaignStatus.Active);
        var inv = NewInvitation(camp.Id, "candidate@acme.test");
        tdb.Db.Campaigns.Add(camp);
        tdb.Db.CampaignInvitations.Add(inv);
        await tdb.Db.SaveChangesAsync();

        var result = await NewService(tdb.NewContext()).JoinCampaignAsync(
            RawTokenOf(inv), "  CANDIDATE@ACME.TEST  ", default);

        Assert.Equal(FixedCandidate, result.CandidateId);
        using var check = tdb.NewContext();
        Assert.Single(await check.CampaignMemberships.ToListAsync());
    }
    ...
    public async Task Join_Duong2_KhongLatRowCV_TaoMembershipTroVeCV()
    {
        ...
        var svc = NewService(tdb.NewContext());
        await svc.JoinCampaignAsync(RawTokenOf(inv), inv.Email, default);
    ...
    public async Task Join_F5_NhanhTaoMoi_SnapshotEmail()
    {
        ...
        await NewService(tdb.NewContext()).JoinCampaignAsync(RawTokenOf(inv), inv.Email, default);
    ...
    public async Task Join_Revoked_410()
    {
        ...
        var svc = NewService(tdb.NewContext());
        await Assert.ThrowsAsync<InvitationGoneException>(() => svc.JoinCampaignAsync(RawTokenOf(inv), inv.Email, default));
    ...
    public async Task Start_TruyenCampaignExpiresAt_XuongSessionClient()
    {
        ...
        session.Verify(x => x.CreateOrGetSessionAsync(
            ...
            It.IsAny<IReadOnlyList<string>>(), It.IsAny<IReadOnlyList<SessionCriterionInput>>(),
            deadline, It.IsAny<bool?>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<string>(), It.IsAny<IReadOnlyList<SessionQuestionInput>?>(),
    ...
    public async Task Start_ExpiresAtNull_TruyenNull_KhongHardDeadline()
    {
        ...
        session.Verify(x => x.CreateOrGetSessionAsync(
            It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<string>(),
            It.IsAny<IReadOnlyList<string>>(), It.IsAny<IReadOnlyList<SessionCriterionInput>>(),
            (DateTime?)null, It.IsAny<bool?>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<string>(), It.IsAny<IReadOnlyList<SessionQuestionInput>?>(),
            It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Start_TruyenCampaignOrgId_XuongSessionClient()
    {
        ...
        session.Verify(x => x.CreateOrGetSessionAsync(
            ...
            It.IsAny<IReadOnlyList<string>>(), It.IsAny<IReadOnlyList<SessionCriterionInput>>(),
            It.IsAny<DateTime?>(), It.IsAny<bool?>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<string>(), It.IsAny<IReadOnlyList<SessionQuestionInput>?>(), It.IsAny<CancellationToken>()), Times.Once);
    }

    // PR160: Start PHẢI truyền đúng campaign.Seniority xuống session client (Interview snapshot sang
    // session). Đây là chốt chặn cho việc đổi THỨ TỰ tham số (CA1068: `seniority` chuyển lên trước
    // `ct`) — hoán vị hai tham số positional là loại lỗi trình biên dịch KHÔNG bắt được khi kiểu khớp,
    // mà mọi mock khác trong file dùng `It.IsAny<string>()` nên sẽ không kêu. Dùng giá trị ≠ mặc định
    // ("Senior", không phải "Junior") để test phân biệt được "truyền đúng" với "rơi về default".
    [Fact]
    public async Task Start_TruyenCampaignSeniority_XuongSessionClient()
    {
        using var tdb = new CampaignTestDb();
        var camp = ActiveCampaignWithQuestionAndCriterion(tdb);
        ...
        session.Verify(x => x.CreateOrGetSessionAsync(
            ...
            "Senior", It.IsAny<IReadOnlyList<SessionQuestionInput>?>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Start_TruyenCampaignAdaptive_XuongSessionClient()
    {
        ...
        session.Verify(x => x.CreateOrGetSessionAsync(
            It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<string>(),
            It.IsAny<IReadOnlyList<string>>(), It.IsAny<IReadOnlyList<SessionCriterionInput>>(),
            It.IsAny<DateTime?>(), true, 2, 8, It.IsAny<int?>(), It.IsAny<string>(), It.IsAny<IReadOnlyList<SessionQuestionInput>?>(), It.IsAny<CancellationToken>()), Times.Once);
    ...
    public async Task Start_CampaignKhongBatAdaptive_TruyenFalse()
    {
        ...
        session.Verify(x => x.CreateOrGetSessionAsync(
            It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<string>(),
            It.IsAny<IReadOnlyList<string>>(), It.IsAny<IReadOnlyList<SessionCriterionInput>>(),
            It.IsAny<DateTime?>(), false, null, null, It.IsAny<int?>(), It.IsAny<string>(), It.IsAny<IReadOnlyList<SessionQuestionInput>?>(), It.IsAny<CancellationToken>()), Times.Once);
    ...
    public async Task Start_TruocCampaignStart_ChanTruocKhiGoiInterview()
    {
        ...
        session.Verify(x => x.CreateOrGetSessionAsync(
            It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<Guid>(), It.IsAny<string>(),
            It.IsAny<IReadOnlyList<string>>(), It.IsAny<IReadOnlyList<SessionCriterionInput>>(),
            It.IsAny<DateTime?>(), It.IsAny<bool?>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<int?>(), It.IsAny<string>(), It.IsAny<IReadOnlyList<SessionQuestionInput>?>(), It.IsAny<CancellationToken>()), Times.Never);
    }
