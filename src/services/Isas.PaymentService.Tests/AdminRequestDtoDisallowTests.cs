...
{
    private static readonly Assembly ServiceAssembly = typeof(AdminCreditsController).Assembly;
    ...
    public void KhoaLa_NemJsonException_KhoaDung_VanParse()
    {
        ...
        Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<GrantCreditRequest>("{\"credits\":1,\"note\":\"x\",\"credit\":2}", opts));
        Assert.NotNull(JsonSerializer.Deserialize<GrantCreditRequest>("{\"credits\":1,\"note\":\"x\"}", opts));
    }
}
