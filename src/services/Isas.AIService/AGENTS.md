...
| POST | `/api/v1/ai/suggest-criterion-levels` | `/api/v1/suggest-criterion-levels` | **Đề xuất MỐC ĐIỂM cho tiêu chí (CAMP-16 / BC-8)** — Gemini, đồng bộ. Lỗi → **ném, KHÔNG fallback dải mặc định** (HR sẽ tin `"Mức 3/10"` là do AI viết) |
| POST | `/api/v1/ai/score-preview` | `/api/v1/score-preview` | **Chấm thử thước đo (CAMP-19)** — viết 3 bài mẫu (yếu/khá/xuất sắc) rồi gọi **CHÍNH `provider.score`** chấm từng bài MỘT LỜI GỌI RIÊNG, `temperature=0.0`, `delivery=None` |
...
`tts`: mp3 vẫn cache theo nội dung ở SeaweedFS/S3; warmup chạy 2 lane để câu 4/5 sẵn sớm hơn. Redis chỉ giữ lease + ready signal TTL để nhiều AIService replica không gọi Gemini trùng. Waiter tối đa 8s rồi fail về FE (không gọi vendor lần hai), lượt vendor tối đa 60s, lease 120s, Redis timeout 250ms + circuit-break 5s và fail-open về single-flight trong process. `TTS_REDIS_ENABLED=false` rollback không rebuild; chi tiết đầy đủ ở [ai.md](../../../docs/services/ai.md) §TTS.
...
`suggest-criterion-levels` *(CAMP-16/BC-8)*: req `{ jobCategory, language, seniority?, jdText?, levelCount?, criteria:[{criterionId, name, description?, maxScore}] }` → res `{ criteria:[{criterionId, levels:[{score, descriptor}]}] }`. Guard phía provider (chỗ DUY NHẤT được tin): drop `criterionId` bịa · drop mốc ngoài `[0,maxScore]` · dedupe · **bắt buộc có mốc `0` và mốc `maxScore`** · descriptor ≥20 ký tự. Descriptor viết **hai vế** `CÓ: <hành vi quan sát được> | CÒN THIẾU: <cái mức trên có mà mức này không>` và **cấm tính từ đánh giá** ("tốt"/"khá") — đó chỉ là đổi tên con số, không thêm thông tin.

`prompt-defaults` *(F21, 2026-09-16, GET, `X-Internal-Token`)*: res `{ defaults: {key: text}, placeholders: {role, job_category} }` — bản MẶC ĐỊNH của từng mảnh prompt admin sửa được, **stateless** (đọc literal từ `prompt_defaults.py`; khe THÊM ⇒ `""`, khe THAY ⇒ chuỗi mẫu có placeholder). Interview kéo về để màn admin hiện "bản đang chạy" (fail-open ⇒ `null`). `test_prompt_defaults.py` khoá ĐỒNG BỘ với builder thật — sửa literal trong `prompts.py`/`seniority.py` mà quên đây ⇒ đỏ.

`score-preview` *(CAMP-19)*: req `{ jobCategory, language, seniority?, question, sampleAnswer?, customAnswer?, targetWordCount, criteria:[…, levels, expectedWeak/Good/Excellent] }` → res `{ samples:[{band, answerText, wordCount, scores[]}], promptVersion?, lengthParityWarning }`.
🔴 **`build_scoring_prompt` và `provider.score` KHÔNG được sửa** — có **golden test khoá theo hash**; đỏ nghĩa là prompt chấm vừa đổi, **cập nhật hash KHÔNG phải cách sửa**. Ba field `expected*` bị **lột trước khi vào prompt chấm** (có test khoá): để lọt là mách đáp án cho chính bộ chấm, khiến mọi con số kỳ-vọng-vs-thật vô nghĩa mà lại trông rất thuyết phục. Luật parity độ dài (3 bài xấp xỉ bằng nhau) là bắt buộc: thiếu nó thì bài test đo **độ dài** chứ không đo thước đo, và nếu bộ chấm cũng thưởng độ dài thì dải điểm đẹp **xác nhận một thước đo hỏng**.
...
| `criteria[]` | **generate-questions (chấm-theo-phạm-vi)**: `{criterionId,name}` — KHÔNG maxScore/weight; có → res thêm `targetCriteria`. ⚠ **KHÔNG còn dùng ở analyze-cv** — sàng CV B2B nay đi `jobNeeds[]` |
...
> ✅ **2026-09-15 — bài giảng roadmap + nạp corpus.** (1) `app/logging_setup.py`: uvicorn/worker KHÔNG tự bật INFO cho `app.*` — trước đó mọi `logger.info` của AIService (kể cả `[⏱]` timing, dòng usage F22, "bài giảng bị trả lại") **chưa bao giờ ra docker logs**; `LOG_LEVEL` (INFO). (2) `/generate-lesson-theory` và `/generate-roadmap` từng là hai đường sinh cuối để Gemini tự quyết thinking → `LESSON_THEORY_THINKING_BUDGET=1024` · `ROADMAP_THINKING_BUDGET=1024` (`-1` quay lui) + `LESSON_THEORY_MAX_OUTPUT_TOKENS=12288` (lưới cho lượt chạy loạn 15–65k token; **suy luận tính vào trần này**). A/B + số đo: `scripts/benchmark-lesson-theory.py`, `docs/services/ai.md`. (3) `provider.embed` chia lô **≤100** text/lời gọi — trần cứng `batchEmbedContents` của Gemini (BK34: trang > 100 chunk từng nạp 502, lỗi bị nuốt). (4) `sanitize_resources` **giữ mục chỉ-tên** khi url trống/host lạ (trước bỏ cả mục, mâu thuẫn với prompt) + ví dụ host **theo ngành** (`resource_host_examples`) + allowlist host BA khai cả `www.` lẫn trần.
...
⚠ **Chỉ `text` nằm trong `required`** của `response_schema` — ép `targetCriterionIds` vào `required` là ép model điền mảng cho MỌI câu, mà rỗng lại hợp lệ ⇒ **đẩy model sang gắn bừa**. **`_keep_known_ids`** = lớp phòng thủ **thứ hai** (giữ id ⊆ tập đã cấp, bỏ trùng, giữ thứ tự), dùng **chung** với `citedChunkIds`; hai tập lọc **độc lập**. **FAIL-OPEN:** thiếu nhãn / toàn id lạ ⇒ `[]`, **KHÔNG raise** (khác `assessments` của sàng CV B2B, chỗ đó raise là đúng vì nó LÀ kết quả sàng lọc) — sinh câu hỏi nằm trên đường **ĐÃ RESERVE CREDIT** (PAY-5), biến nhãn phụ thành đường làm hỏng cả buổi thì đắt hơn nhiều.
