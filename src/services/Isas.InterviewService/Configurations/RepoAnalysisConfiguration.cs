...
public class RepoAnalysisConfiguration : IEntityTypeConfiguration<RepoAnalysis>
{
    ...
    public void Configure(EntityTypeBuilder<RepoAnalysis> e)
    {
        ...
        e.Property(x => x.Languages).HasConversion(v => JsonSerializer.Serialize(v, Json), v => JsonSerializer.Deserialize<Dictionary<string,long>>(v, Json) ?? new Dictionary<string,long>()).HasColumnType("jsonb").IsRequired();
